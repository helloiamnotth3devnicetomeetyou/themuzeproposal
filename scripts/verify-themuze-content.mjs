...
const [albums, members, tracks, gallery, notices, schedules] =
  await Promise.all([
    ...
    rows(
      "artist_schedules",
      "id,title_en,title_ja,location_en,location_ja,link_url",
    ),
  ]);
...
assert.equal(
  albums.filter((album) => album.youtube_url).length,
  10,
  "every album needs a representative YouTube link",
);
for (const [albumTitle, trackTitle] of expectedVideos) {
  const track = tracks.find(
    (item) =>
      albumsById.get(item.album_id)?.title === albumTitle &&
      item.title === trackTitle,
  );
  assert(
    track?.youtube_url && track.music_video_url,
    `${albumTitle} / ${trackTitle} MV is missing`,
  );
}
const albumIds = new Map(albums.map(({ title, id }) => [title, id]));
const memberIds = new Map(members.map(({ slug, id }) => [slug, id]));
const publishedGalleryPairs = new Map(
  gallery
    .filter(({ is_published }) => is_published)
    .map((item) => [`${item.album_id}:${item.member_id}`, item]),
);
assert.equal(albumIds.size, 10, "10 albums are required");
assert.equal(memberIds.size, 5, "5 members are required");
const translatedNotices = notices.filter(
  (notice) =>
    notice.title_en &&
    notice.title_ja &&
    notice.content_en &&
...
assert(
  translatedNotices.length >= 12,
  "at least 12 translated curated notices are required",
);
const importedSchedules = schedules.filter(({ id }) =>
  id.startsWith("a1300000-"),
);
assert.equal(
  importedSchedules.length,
  10,
  "10 official schedules are required",
);
assert(
  importedSchedules.every(
    (item) =>
      item.title_en &&
      item.title_ja &&
      item.location_en &&
  ...
  "official schedules need translations, location, and source link",
);
for (const { image_url: imageUrl } of publishedGalleryPairs.values()) {
  ...
  assert(
    ...
    `unreachable image: ${imageUrl}`,
  );
}

console.log(
  "Verified official MV links, published gallery photos, 12 translated notices, and 10 translated schedules.",
);
