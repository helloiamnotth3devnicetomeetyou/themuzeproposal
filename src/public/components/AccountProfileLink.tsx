...
const copy: Record<Locale, { profileLabel: (name: string) => string; remaining: (count: number) => string; quotaInfo: string }> = {
  ko: { profileLabel: (name) => `${name} 프로필`, remaining: (count) => `오늘 제출 가능 ${count}회`, quotaInfo: "하루 5회 제출할 수 있습니다." },
  en: { profileLabel: (name) => `${name} profile`, remaining: (count) => `${count} submission${count === 1 ? "" : "s"} remaining today`, quotaInfo: "You can submit up to 5 times per day." },
  ja: { profileLabel: (name) => `${name}のプロフィール`, remaining: (count) => `本日あと${count}回送信できます`, quotaInfo: "1日に5回まで送信できます。" },
};
...
export default function AccountProfileLink({ name, email, avatarUrl, remaining }: { name: string; email: string; avatarUrl: string; remaining: number }) {
  ...
  return <section className={styles.accountBlock}>
    <Link href="/account" className={styles.accountProfile} aria-label={pageCopy.profileLabel(label)}>
    ...
    <div className={styles.quota} aria-label={pageCopy.remaining(remaining)}>
      ...
      <span className={styles.quotaInfo} tabIndex={0}>i<span role="tooltip">{pageCopy.quotaInfo}</span></span>
    </div>
  </section>;
}
