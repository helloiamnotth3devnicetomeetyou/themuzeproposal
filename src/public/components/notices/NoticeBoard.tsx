...
export default function NoticeBoard({
  ...
  artistSlug?: string;
  initialData: NoticeListDTO | null;
  loadFailed?: boolean;
}) {
  ...
  const categories = useMemo(
    () =>
      Array.from(
        new Set(
          notices
            .map((notice) => localized(notice.category, locale))
            .filter(Boolean),
        ),
      ),
    [notices, locale],
    ...
    return notices
      ...
      .filter(({ notice }) => {
        ...
        const matchesCategory =
          selectedCategory === "all" || noticeCategory === selectedCategory;
        const matchesSearch =
          !keyword ||
          `${localized(notice.title, locale)} ${noticeCategory}`
            .toLocaleLowerCase()
            .includes(keyword);
  ...
  const detailHref = (noticeId: string) =>
    artistSlug ? `/${artistSlug}/notice/${noticeId}` : `/notice/${noticeId}`;
  const closeSearch = () => {
    setSearch("");
    setSearchOpen(false);
  };
      ...
      <section className={styles.board} aria-labelledby="notice-heading">
        <header className={styles.titleColumn}>
          <div className={styles.titleSticky}>
            <h1
              id="notice-heading"
              className={artistSlug ? styles.artistHeading : undefined}
            >
              {artistSlug && scopeName ? (
                <>
                  <span className={styles.artistNameLine}>
                    {scopeName.toUpperCase()}
                  </span>
                  <span className={styles.noticeLine}>NOTICE</span>
                </>
              ) : (
                heading
              )}
            ...
            <span className={styles.total}>
              {String(notices.length).padStart(2, "0")} {copy.count}
            </span>
          </div>
        </header>
        ...
        <div className={styles.listColumn}>
          <div className={styles.toolbar}>
            ...
            />
            <div
              className={`${styles.searchControl} ${searchOpen ? styles.searchOpen : ""}`}
            >
          ...
          <div className={styles.list}>
            ...
            {!loading &&
              !error &&
              paginatedNotices.map(({ notice, number }) => (
                <Link
                  key={notice.id}
                  href={detailHref(notice.id)}
                ...
                >
                  ...
                  <span className={styles.itemCopy}>
                    <span className={styles.itemCategory}>
                      {localized(notice.category, locale) || copy.all}
                    </span>
            ...
            {!loading && !error && !visibleNotices.length && (
