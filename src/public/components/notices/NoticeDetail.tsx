...
import type {
  ...
  NoticeDetailDTO,
  NoticeNavigationDTO,
} from "@/public/features/notices/types";
...
const localized = (value: LocalizedTextDTO, locale: Locale) =>
  value[locale] || value.ko || value.en || value.ja;
...
export default function NoticeDetail({
  ...
  noticeId: string;
  artistSlug?: string;
  initialData: NoticeDetailDTO | null;
  initialNavigation?: NoticeNavigationDTO | null;
  loadFailed?: boolean;
}) {
  ...
  const detailHref = (id: string) =>
    artistSlug ? `/${artistSlug}/notice/${id}` : `/notice/${id}`;

  return (
    <div className={styles.pageFrame}>
      <section
        className={`${styles.board} ${styles.detailBoard}`}
      ...
      >
        <header className={styles.titleColumn}>
          <div className={styles.titleSticky}>
            ...
            {!preview && initialNavigation && (
              <nav
                className={styles.detailNavigation}
                aria-label={pageCopy.detailNav}
              >
                {initialNavigation.previous && (
                  <Link href={detailHref(initialNavigation.previous.id)}>
                    <span>
                      {pageCopy.previous}
                      <ArrowLeft aria-hidden="true" />
                    </span>
                    <b>{localized(initialNavigation.previous.title, locale)}</b>
                  </Link>
                )}
                {initialNavigation.next && (
                  <Link href={detailHref(initialNavigation.next.id)}>
                    <span>
                      {pageCopy.next}
                      <ArrowRight aria-hidden="true" />
                    </span>
                    <b>{localized(initialNavigation.next.title, locale)}</b>
                  </Link>
                )}
              </nav>
            )}
        ...
        <div className={`${styles.listColumn} ${styles.articleColumn}`}>
          ...
          {!loading && error && (
            <div className={`${styles.state} ${styles.error}`} role="alert">
              <b>!</b>
              <p>{error}</p>
              <Link href={listHref}>{pageCopy.back}</Link>
            </div>
          )}
          {!loading && notice && (
            <article className={styles.article}>
              ...
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: sanitizeRichText(localized(notice.content, locale)),
                }}
              />
              <Link href={listHref} className={styles.articleBack}>
                <ArrowLeft aria-hidden="true" />
                {pageCopy.back}
              </Link>
            </article>
