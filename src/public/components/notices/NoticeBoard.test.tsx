...
vi.mock("@/styles/(public)/components/notices/NoticeBoard.module.css", () => ({
  default: {},
}));
...
const makeData = (notices: NoticeDTO[], name = ""): NoticeListDTO => ({
  name,
  notices,
});
...
describe("NoticeBoard", () => {
  ...
  it("renders each notice as a link", () => {
    const notices = [
      makeNotice("abc", "첫번째 공지"),
      makeNotice("def", "두번째 공지"),
    ];
    render(<NoticeBoard initialData={makeData(notices)} />);
    expect(screen.getByRole("link", { name: /첫번째 공지/ })).toHaveAttribute(
      "href",
      "/notice/abc",
    );
    expect(screen.getByRole("link", { name: /두번째 공지/ })).toHaveAttribute(
      "href",
      "/notice/def",
    );
  });
  ...
  it("uses artistSlug in detail href", () => {
    ...
    render(
      <NoticeBoard
        artistSlug="rescene"
        initialData={makeData(notices, "rescene")}
      />,
    );
  ...
  it("shows empty state when no notices", () => {
    render(<NoticeBoard initialData={makeData([])} />);
    expect(
      screen.getByText("조건에 맞는 공지가 없습니다."),
    ).toBeInTheDocument();
  });
  ...
  it("filters notices by search input", () => {
    ...
    expect(
      screen.getByRole("link", { name: /이벤트 공지/ }),
    ).toBeInTheDocument();
