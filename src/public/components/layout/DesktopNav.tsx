...
export default function DesktopNav({
  ...
  return (
    <div
      className={`hidden w-full items-center xl:flex ${authReady ? "" : styles.authLoading}`}
    >
      <Link
        href="/"
        prefetch={false}
        className="relative block h-10 w-40 shrink-0 transition-opacity duration-base hover:opacity-75"
      >
        <Image
          src="/images/logo.png"
          alt="THE MUZE Logo"
      ...
      >
        ...
        {(artists || []).map((artist) => (
          ...
          >
            <button
              ...
              className={`${styles.navLink} ${expandedArtist === artist.slug || pathname.startsWith(`/${artist.slug}/`) ? styles.navLinkActive : ""}`}
            ...
            <div
              className={`absolute left-1/2 top-full z-50 w-44 -translate-x-1/2 pt-2 transition-all duration-base ${expandedArtist === artist.slug ? "visible translate-y-0 opacity-100" : "invisible -translate-y-1 opacity-0"}`}
            >
              <div className={styles.dropdownCard}>
                {artistLinks(artist.slug).map(({ href, label }) => (
                  <Link
                    key={href}
                    href={href}
                    prefetch={false}
        ...
        <Link
          href="/audition"
          prefetch={false}
          className={linkClass("/audition")}
        >
          {t.nav.audition}
        </Link>
        <Link href="/notice" prefetch={false} className={linkClass("/notice")}>
          {t.nav.notice}
        </Link>
        <Link
          href="/protect"
          prefetch={false}
          className={linkClass("/protect")}
        >
          PROTECT
        </Link>
        <Link
          ...
          <div className={styles.themeToggleIcons}>
            <Sun
              ...
              className={
                !isDark
                  ? "opacity-0"
                  : "opacity-60 transition-opacity duration-slow"
              }
            />
            <Moon
              size={12}
