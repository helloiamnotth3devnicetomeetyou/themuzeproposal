...
export default function Navbar({ initialArtists = [] }: { initialArtists?: ArtistNavigationItem[] }) {
  ...
  useEffect(() => { let active = true; const load = async () => { const { supabase } = await import("@/core/supabase/client"); const { data } = await supabase.from("artists").select("id, slug, name, logo_url").order("name", { ascending: true }); if (active && data) setArtists(data); }; if (!initialArtists?.length) void load(); window.addEventListener(ARTISTS_CHANGED_EVENT, load); return () => { active = false; window.removeEventListener(ARTISTS_CHANGED_EVENT, load); }; }, [initialArtists?.length]);
  ...
  const shared = { artists: artists || [], pathname, isAdmin, isLoggedIn, isDark, t, onToggleTheme: toggleTheme };
