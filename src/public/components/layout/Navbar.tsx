...
export default function Navbar() {
  ...
  useEffect(() => { let active = true; const load = async () => { const { data } = await supabase.from("artists").select("id, slug, name, logo_url").order("name", { ascending: true }); if (active && data) setArtists(data); }; void load(); window.addEventListener(ARTISTS_CHANGED_EVENT, load); return () => { active = false; window.removeEventListener(ARTISTS_CHANGED_EVENT, load); }; }, []);
  ...
  const shared = { artists, pathname, isAdmin, isLoggedIn, isDark, t, onToggleTheme: toggleTheme };
