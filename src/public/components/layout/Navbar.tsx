...
export default function Navbar({ initialArtists = [] }: { initialArtists?: ArtistNavigationItem[] }) {
  ...
  useEffect(() => { void (async () => { const { supabase } = await import("@/core/supabase/client"); const { data: { session } } = await supabase.auth.getSession(); const user = session?.user; setIsLoggedIn(Boolean(user)); if (user) { const { isAdmin: checkIsAdmin } = await import("@/core/auth/auth"); setIsAdmin(await checkIsAdmin(user.id)); } else setIsAdmin(false); })(); }, [pathname]);
