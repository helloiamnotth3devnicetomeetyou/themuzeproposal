...
export default function Navbar({ initialArtists = [] }: { initialArtists?: ArtistNavigationItem[] }) {
  ...
  useEffect(() => { void (async () => { const { getUser, isAdmin: checkIsAdmin } = await import("@/core/auth/auth"); const user = await getUser(); setIsLoggedIn(Boolean(user)); setIsAdmin(user ? await checkIsAdmin(user.id) : false); })(); }, [pathname]);
