...
export default function ArtistSceneExperience({ artistSlug, initialMemberSlug, initialData = null }: { artistSlug: string; initialMemberSlug?: string; initialData?: ArtistSceneData | null }) {
  ...
  const { data, loading, error, loadSceneMembers } = useArtistSceneData({ artistSlug, profilePreview, memberPreview, initialData });
  ...
  const selectedRegionCentroid = useMemo(() => selectedRegion ? outlineCentroid(selectedRegion.outline) : { x: 50, y: 50 }, [selectedRegion]);
  ...
  const selectMember = (memberId: string) => { const member = members.find((item) => item.id === memberId); if (!member || !artist) return; const matchingScene = activeScene?.member_ids?.includes(memberId) ? activeScene : scenes.find((scene) => scene.member_ids?.includes(memberId)); if (matchingScene) setActiveSceneId(matchingScene.id); const path = `/${artist.slug}/artist/${member.slug}${window.location.search}`; if (!selectedMemberId) { window.history.pushState({ artistMemberFocus: true }, "", path); focusWasOpenedHere.current = true; } else window.history.replaceState({ artistMemberFocus: true }, "", path); setSelectedMemberId(memberId); setGroupFocused(false); setHoveredMemberId(null); };
  const closeMember = () => { if (!artist) return; if (focusWasOpenedHere.current) { focusWasOpenedHere.current = false; window.history.back(); return; } window.history.replaceState(null, "", `/${artist.slug}/artist${window.location.search}`); setSelectedMemberId(null); setGroupFocused(false); setHoveredMemberId(null); };
  const reset = () => { if (!artist) return; focusWasOpenedHere.current = false; pendingSceneId.current = null; window.history.replaceState(null, "", `/${artist.slug}/artist${window.location.search}`); setSelectedMemberId(null); setGroupFocused(false); setHoveredMemberId(null); setActiveSceneId(scenes[0]?.id ?? ""); };
  ...
  return <main className={`${styles.experience} ${selectedMember || groupFocused ? styles.hasSelection : ""}`} style={{ "--artist-accent": selectedMember?.color || artist.color || BRAND_PINK_HEX } as CSSProperties}>
    ...
    {!selectedMember && <div className={`${styles.artistIdentity} ${groupFocused ? styles.artistIdentityFocused : ""}`}><button type="button" className={styles.artistWordmark} onClick={(event) => { event.stopPropagation(); setHoveredMemberId(null); setSelectedMemberId(null); setGroupFocused((current) => !current); }} aria-label={`${artistName} ${copy.profile}`} aria-expanded={groupFocused} aria-controls={groupBio ? "group-artist-bio" : undefined}>{artist.logo_url && <Image src={artist.logo_url} alt={`${artistName} logo`} width={240} height={80} /> }<h1>{artistName}</h1><span className={styles.artistProfileToggle} aria-hidden="true" /></button>{groupBio && <div id="group-artist-bio" className={styles.artistBioReveal} aria-hidden={!groupFocused}><div dangerouslySetInnerHTML={{ __html: sanitizeRichText(groupBio) }} /></div>}</div>}
    {selectedMember && <MemberDetailOverlay member={selectedMember} memberBio={memberBio || ""} panelLeft={selectedRegionCentroid.x > 56} copy={copy} onClose={closeMember} onNavigate={navigate} />}
