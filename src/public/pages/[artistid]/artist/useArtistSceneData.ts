...
async function getSceneMembers(sceneId: string) {
  const result = await supabase.from("artist_scene_members").select(regionSelect).eq("scene_id", sceneId).order("sort_order");
  if (result.error) throw result.error;
  return (result.data ?? []).map((region) => ({ ...region, outline: normalizeOutline(region.outline) })).filter((region) => region.outline.length >= 3);
}
...
export function useArtistSceneData({ artistSlug, profilePreview, memberPreview, initialData = null }: { artistSlug: string; profilePreview: unknown; memberPreview: unknown; initialData?: ArtistSceneData | null }) {
  ...
  const loadedSceneIds = useRef(new Set(initialData?.scenes.filter((scene) => !scene.member_ids?.length).map((scene) => scene.id)));
  ...
  const loadSceneMembers = useCallback(async (sceneId: string) => {
    if (loadedSceneIds.current.has(sceneId)) return;
    const scene = data?.scenes.find((item) => item.id === sceneId);
    if (!scene?.member_ids?.length) { loadedSceneIds.current.add(sceneId); return; }
    const regions = await getSceneMembers(sceneId);
    loadedSceneIds.current.add(sceneId);
    setData((current) => current && ({ ...current, scenes: current.scenes.map((scene) => scene.id === sceneId ? normalizeScene({ ...scene, artist_scene_members: regions }) : scene) }));
  }, [data]);
