...
export default function SceneCanvas({ scene, members, artistName, sceneLabel, focusMemberId, groupFocused, selectedMember, cameraOffset, onClose, onHover, onSelect }: Props) {
  const regions = useMemo(() => scene.artist_scene_members.map((region) => ({ ...region, member: members.find((member) => member.id === region.member_id), path: outlineToPath(region.outline) })), [scene.artist_scene_members, members]);
  ...
  return <>
    ...
    <section className={styles.sceneViewport} aria-label={`${artistName} ${scene.title || sceneLabel}`}><div className={styles.sceneStage}><div className={styles.sceneFrame} onClick={onClose} style={{ "--scene-ratio": ratio, transformOrigin: "50% 50%", transform: selectedMember ? `translate(${cameraOffset.x}%, ${cameraOffset.y}%) scale(1.16)` : "translate(0, 0) scale(1)" } as CSSProperties}>
      ...
      <svg className={styles.hitMap} viewBox="0 0 100 100" preserveAspectRatio="none" aria-label={sceneLabel}>{regions.map(({ id, member, path }) => { if (!member || !path) return null; return <path key={id} d={path} className={`${styles.hitRegion} ${groupFocused || focusMemberId === member.id ? styles.isFocusedRegion : ""}`} role="button" tabIndex={0} aria-label={member.name} onPointerEnter={() => onHover(member.id)} onPointerLeave={() => onHover(null)} onFocus={() => onHover(member.id)} onBlur={() => onHover(null)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(member.id); } }} onClick={(event) => { event.stopPropagation(); onSelect(member.id); }} />; })}</svg>
    </div></div></section>
  </>;
}
