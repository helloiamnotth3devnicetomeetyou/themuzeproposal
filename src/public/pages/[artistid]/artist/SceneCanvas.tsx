  ...
  const regions = useMemo(
    () =>
      scene.artist_scene_members.map((region) => ({
        ...region,
        member: members.find((member) => member.id === region.member_id),
        path: outlineToPath(region.outline),
      })),
    [scene.artist_scene_members, members],
  );
  const focusRegions = groupFocused
    ? scene.artist_scene_members
    : focusMemberId
      ? scene.artist_scene_members.filter(
          (region) => region.member_id === focusMemberId,
        )
      : [];
  ...
  return (
    <>
      ...
      <section
        ...
        aria-label={`${artistName} ${scene.title || sceneLabel}`}
      >
        <div className={styles.sceneStage}>
            ...
            style={
              {
                ...
                transform: selectedMember
                  ? `translate(${cameraOffset.x}%, ${cameraOffset.y}%) scale(1.16)`
          ...
          >
            <Image
              key={scene.id}
              src={scene.image_url}
              alt={`${artistName} ${scene.title || sceneLabel}`}
              width={imageWidth}
              height={imageHeight}
              preload
              fetchPriority="high"
                ...
                >
                  <defs>
                    ...
                    >
                      {region.mask_url ? (
                        <image
                          href={region.mask_url}
                          x="0"
                          y="0"
                          width="100"
                          height="100"
                          preserveAspectRatio="none"
                          filter={`url(#feather-${region.id})`}
                        />
                      ) : (
                        <path
                          d={path}
            ...
            >
              {regions.map(({ id, member, path }) => {
                if (!member || !path) return null;
                return (
                  <path
                    key={id}
                    d={path}
                    className={`${styles.hitRegion} ${groupFocused || focusMemberId === member.id ? styles.isFocusedRegion : ""}`}
                    role="button"
                    tabIndex={0}
