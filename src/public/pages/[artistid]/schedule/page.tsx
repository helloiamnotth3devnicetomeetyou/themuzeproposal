...
const daysUntil = (value: string) => {
  ...
  return Math.round((dateAtLocalMidnight(value).getTime() - today.getTime()) / 86_400_000);
};
...
export default function ArtistSchedulePage() {
  ...
  useEffect(() => {
    let cancelled = false;
    const scheduleColumns = "id,event_date,start_time,category,title_ko,title_en,title_ja,description_ko,description_en,description_ja,location,location_ko,location_en,location_ja,link_url";
    async function load() {
      setLoading(true);
      if (previewArtistId) {
      ...
      const joined = await supabase
        ...
        .order("start_time", { foreignTable: "artist_schedules", ascending: true, nullsFirst: true })
      ...
      if (joined.error?.message.includes("location_ko")) {
        ...
        const artistResult = await supabase.from("artists").select("id,color").eq("slug", artistid).eq("is_active", true).maybeSingle();
        if (cancelled) return;
        ...
        if (cancelled) return;
        if (legacy.error) setError(legacy.error.message.includes("artist_schedules") ? t.schedule.tableMissing : t.schedule.loadError);
        else setEvents((legacy.data?.map((row) => ({ ...row, location_ko: row.location, location_en: null, location_ja: null })) ?? []) as ScheduleRow[]);
        setLoading(false);
        return;
      }
      if (joined.error) {
        setError(joined.error.message.includes("artist_schedules") ? t.schedule.tableMissing : t.schedule.artistNotFound);
        setLoading(false);
        return;
      }
      if (!joined.data) {
      ...
      setEvents((joined.data.artist_schedules ?? []) as unknown as ScheduleRow[]);
      setLoading(false);
    }
    void load();
    ...
    <main className={styles.page} style={{ "--artist-color": artistColor } as CSSProperties}>
      <div className={styles.layout}>
            ...
            {visibleEvents.map((event) => {
              ...
              return <article key={event.id} id={`schedule-event-${event.id}`} className={`${styles.event} ${isPast ? styles.eventPast : ""} ${isSelected ? styles.eventSelected : ""}`} style={eventStyle}>
                ...
                <div className={styles.eventMeta}>
                  <button type="button" className={styles.eventType} aria-pressed={categoryFilters.includes(event.category)} onClick={() => toggleCategory(event.category)}><i><CategoryIcon aria-hidden="true" /></i>{t.schedule.categories[event.category] ?? t.schedule.categories.etc}</button>
                  {remaining >= 0 && <span className={styles.eventCountdown}>{remaining ? `D-${remaining}` : "D-DAY"}</span>}
                  {event.start_time && <b>{event.start_time.slice(0, 5)}</b>}
                  {localizedLocation(event) && <b>{localizedLocation(event)}</b>}
                </div>
                {isSelected && <div className={styles.eventDetail}>
                  {localized(event, "description") && <p>{localized(event, "description")}</p>}
                  {href && <a href={href} target="_blank" rel="noreferrer">{t.schedule.categories[event.category] ?? t.schedule.categories.etc} {"→"}</a>}
                </div>}
              </article>;
            })}
            {!visibleEvents.length && !error && <div className={styles.empty}>{t.schedule.empty}</div>}
          </div>
          {filteredMonthEvents.length > PAGE_SIZE && <div className={styles.pager} aria-label={t.schedule.pageLabel}>
        ...
        <section className={styles.calendarPanel} aria-label={t.schedule.calendarLabel(cursor.getFullYear(), cursor.getMonth() + 1)}>
          ...
          <div className={styles.legend} aria-label={t.schedule.eventTypes}>{(Object.keys(CATEGORIES) as Category[]).map((key) => { const CategoryIcon = CATEGORIES[key].icon; return <button key={key} type="button" aria-pressed={categoryFilters.includes(key)} onClick={() => toggleCategory(key)} style={{ "--event-color": CATEGORIES[key].color } as CSSProperties}><i><CategoryIcon aria-hidden="true" /></i>{t.schedule.categories[key]}</button>; })}</div>
        </section>
      </div>
    </main>
  );
