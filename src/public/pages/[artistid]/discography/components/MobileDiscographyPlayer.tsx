...
export function MobileDiscographyPlayer({
  ...
  const selectAlbum = (index: number) => {
    ...
    const track = albumTrackRef.current;
    if (track) {
      track.style.transition = window.matchMedia("(prefers-reduced-motion: reduce)").matches
        ? "none"
    ...
    <section className="w-full font-[family-name:var(--font-sans)]">
      ...
      {view === "album" ? (
        <div id="discography-album-panel" role="tabpanel" className="pb-8 pt-3">
          ...
          >
            <div ref={albumTrackRef} className="mobile-album-track flex w-[300%] will-change-transform">
              <div className="w-1/3 shrink-0 px-4" aria-hidden="true">
                <div className="relative mx-auto aspect-square w-full max-w-[390px] overflow-hidden rounded-[1.35rem]">
                  {previousAlbum && <Image src={previousAlbum.cover} alt="" fill loading="eager" sizes="(max-width: 640px) calc(100vw - 72px), 390px" className="object-cover" />}
                </div>
              </div>
              ...
              <div className="w-1/3 shrink-0 px-4" aria-hidden="true">
                <div className="relative mx-auto aspect-square w-full max-w-[390px] overflow-hidden rounded-[1.35rem]">
                  {nextAlbum && <Image src={nextAlbum.cover} alt="" fill loading="eager" sizes="(max-width: 640px) calc(100vw - 72px), 390px" className="object-cover" />}
                </div>
              </div>
            </div>
          </div>
