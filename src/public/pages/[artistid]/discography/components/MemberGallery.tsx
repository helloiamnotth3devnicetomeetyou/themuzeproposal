...
export const MemberGallery = memo(function MemberGallery({ album, members, gallery, albumColor, layout = "panel" }: MemberGalleryProps) {
  ...
  const albumGallery = useMemo(() => gallery.filter((item) => !item.albumId || item.albumId === album.id), [album.id, gallery]);
  const filteredGallery = useMemo(() => selectedMemberId === "all" ? albumGallery : albumGallery.filter((item) => item.memberId === selectedMemberId), [albumGallery, selectedMemberId]);
  const memberMap = useMemo(() => new Map(members.map((member) => [member.id, member])), [members]);
  const photoCounts = useMemo(() => {
    const counts = new Map<string, number>();
    albumGallery.forEach((item) => item.memberId && counts.set(item.memberId, (counts.get(item.memberId) || 0) + 1));
    return counts;
  }, [albumGallery]);
