  ...
  if (!gallery.length)
    return (
      ...
      >
        ...
        />
        <p className="text-xs text-[var(--palette-9ca3af)]">
          {t.discography.noPhotos}
        </p>
      </div>
    );
  return (
      ...
      {gallery.map((item, index) => {
        ...
        return (
          <button
            ...
            className={`group relative rounded-xl overflow-hidden cursor-pointer border transition-all duration-300 hover:z-20 hover:scale-[1.03] ${bentoSpan(index, gallery.length)}`}
            style={{
