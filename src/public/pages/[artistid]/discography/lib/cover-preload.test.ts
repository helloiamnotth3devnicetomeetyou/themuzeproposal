...
describe("coverPreloadQueue", () => {
  ...
  it("preloads only the selected album's member images", () => {
    const item = (id: string, albumId: string) => ({ id, albumId, imageUrl: `/images/${id}.jpg`, caption: "", sortOrder: 1 });
    const queue = galleryPreloadQueue([item("a", "album-a"), item("b", "album-b")], "album-a");
