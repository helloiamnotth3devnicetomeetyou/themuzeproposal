...
const album = (id: string, releaseDate: string): DiscographyAlbum => ({ id, title: id, titles: {}, type: "", releaseDate, cover: "", color: "", tracks: [], desc: { ko: "", en: "", ja: "" }, links: {} });
...
describe("newestAlbumsFirst", () => {
  it("returns URL and playback lookup order without mutating fetched albums", () => {
    ...
    expect(newestAlbumsFirst(fetched).map((item) => item.id)).toEqual(["new", "old"]);
