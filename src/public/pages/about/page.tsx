...
export default function About() {
  ...
  useEffect(() => {
    ...
    const loadAboutContent = async () => {
      const noticesResult = await supabase
        .from("notices")
        .select(
          "id,date,title_ko,title_en,title_ja,category_ko,category_en,category_ja",
        )
  ...
  const noticeList = notices.map((notice) => ({
    ...
    category: localizeText(
      {
        ko: notice.category_ko,
        en: notice.category_en,
        ja: notice.category_ja,
      },
      locale,
    ),
  }));
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-3 pt-4">
          ...
          {!noticesLoading && !noticeList.length && (
            <div className="p-5 text-sm" style={{ color: "var(--text-muted)" }}>
