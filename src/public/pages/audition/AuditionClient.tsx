  ...
  return <main className={protectStyles.page}><section className={protectStyles.shell}>
    ...
    <div className={protectStyles.contentColumn}>
      ...
      {activeTab === "apply" && <section className={styles.applicationPanel}>
        ...
        {!campaign ? <div className={protectStyles.emptyState}><p>{m.noCampaign}</p><span>{m.noCampaignDescription}</span></div> : <><div className={styles.campaignIntro}><span className={styles.campaignStatus}>OPEN</span><h2>{campaign.title}</h2><p>{campaignDescription(campaign, locale)}</p>{campaign.starts_at && <time dateTime={campaign.starts_at}>{m.opensOn(new Intl.DateTimeFormat(localeTags[locale], { dateStyle: "long" }).format(new Date(campaign.starts_at)))}</time>}{campaign.ends_at && <time dateTime={campaign.ends_at}>{m.deadline(new Intl.DateTimeFormat(localeTags[locale], { dateStyle: "long" }).format(new Date(campaign.ends_at)))}</time>}</div><CampaignFormClient key={`${campaign.id}-${editing?.id || "new"}`} campaign={campaign} fields={initialFields.filter((field) => field.campaign_id === campaign.id)} initialSubmission={editing?.campaign_id === campaign.id ? editing : null} userEmail={userEmail} onSaved={(submission, nextRemaining) => { setSubmissions((current) => [submission, ...current.filter((item) => item.id !== submission.id)]); setEditingId(submission.id); setRemaining(nextRemaining); }} onViewMine={() => setActiveTab("mine")} /></>}
      </section>}
    </div>
  </section></main>;
}
