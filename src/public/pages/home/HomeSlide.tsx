          ...
          <video
            ...
            style={
              slide.imageUrl
                ? undefined
                : {
                    opacity: readyVideoSlideIds.has(slide.id) ? 1 : 0,
      ...
      {shouldLoadMedia &&
        ...
        (index !== 0 ||
          ...
          <Image
            ...
            style={{
              animation: isVisible
                ? "kenBurnsIn 8s ease-out forwards"
                : undefined,
            }}
          />
        )}
        ...
        <div className="home-hero-copy">
          <span className="home-release-meta">
            ...
            {slide.type && (
              <>
                <span
                  style={{ color: "var(--alpha-ffffff-3)", margin: "0 0.4em" }}
                >
                  ·
                </span>
                <span style={{ color: "var(--color-static-white)" }}>
                  {slide.type}
                </span>
              </>
            )}
          ...
          <h2 className="home-release-title" aria-label={slide.title}>
            {isVisible && slide.typoLogoUrl ? (
              <TypoLogoMask
                src={slide.typoLogoUrl}
                label={slide.title}
          ...
          <div className="home-release-actions">
            ...
            {(slide.youtubeUrl || slide.spotifyId) && (
              <StreamingActions
                slide={slide}
                open={isStreamingOpen}
                listenLabel={listenLabel}
                onToggle={() => onStreamingToggle(slide.id)}
                onClose={onStreamingClose}
              />
            )}
          </div>
        </div>
...
function StreamingActions({
  ...
  return (
    <div className={`home-stream-actions ${open ? "is-open" : ""}`}>
      <button
        type="button"
        aria-expanded={open}
        aria-controls={`streaming-${slide.id}`}
        onClick={onToggle}
        className={`home-listen-trigger ${open ? "is-open" : ""}`}
      >
        <span className="home-listen-icon" aria-hidden="true">
          <Headphones />
        </span>
      ...
      <div
        ...
        className={`home-stream-platforms ${open ? "is-open" : ""}`}
        aria-hidden={!open}
      >
        {slide.youtubeUrl && (
          <StreamingLink
            href={slide.youtubeUrl}
            label={`${slide.title} on YouTube`}
        ...
        {spotifyAlbumHref(slide.spotifyId) && (
          <StreamingLink
            href={spotifyAlbumHref(slide.spotifyId)!}
            label={`${slide.title} on Spotify`}
...
function videoStartTime(videoUrl: string) {
  const start = Number(new URL(videoUrl).hash.match(/^#t=([\d.]+)/)?.[1]);
...
function SpotifyIcon({ className, ...props }: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" className={className} {...props}>
      ...
      <path
        d="M6.7 9.8c3.5-1 7.3-.6 10.4 1M7.3 13c3-0.8 6.3-.5 9 1M8 16c2.5-.6 5.1-.3 7.2.9"
        stroke="var(--color-static-black)"
        strokeWidth="1.5"
        strokeLinecap="round"
      />
    </svg>
