...
export default function Home({
  ...
  const [openStreamingSlideId, setOpenStreamingSlideId] = useState<
    string | null
  >(null);
  const [isFirstImageLoaded, setIsFirstImageLoaded] = useState(
    !rawSlides[0]?.imageUrl,
  );
  const [readyVideoSlideIds, setReadyVideoSlideIds] = useState<Set<string>>(
    () => new Set(),
  );
  const [failedVideoSlideIds, setFailedVideoSlideIds] = useState<Set<string>>(
    () => new Set(),
  );
  ...
  const nextVideoIndex = slides.findIndex(
    (slide, index) =>
      index > 0 &&
      slide.videoUrl &&
      !readyVideoSlideIds.has(slide.id) &&
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      setReadyVideoSlideIds((current) =>
        current.has(firstSlide.id)
          ? current
          : new Set(current).add(firstSlide.id),
      );
  ...
  useEffect(() => {
    ...
    const syncVisibility = () => {
      ...
      if (!visible)
        document
          .querySelectorAll<HTMLVideoElement>(".home-hero-video")
  ...
  const goToSlide = useCallback(
    (next: number) => {
      ...
      const nextTransition = startSlideTransition(
        currentSlide,
        next,
        slides.length,
      );
    ...
    document
      ...
      .forEach((video) => {
        ...
        if (
          ...
          Math.abs(video.currentTime - startTime) > 0.1
        ) {
          const onSeeked = () => {
  ...
  useEffect(() => {
    ...
    const remaining = Math.max(
      0,
      AUTOPLAY_DURATION - autoplayElapsedRef.current,
    );
    ...
    return () => {
      if (autoplayTimeout.current !== null)
        clearTimeout(autoplayTimeout.current);
      if (autoplayStartedAt.current !== null) {
        const elapsed = Math.min(
          AUTOPLAY_DURATION,
          autoplayElapsedRef.current +
            performance.now() -
            autoplayStartedAt.current,
        );
    ...
    return (
      <main className="relative flex h-[100dvh] w-full items-center justify-center overflow-hidden bg-[var(--bg-base)] transition-colors duration-slow">
        <div className="text-center">
          <p className="font-display text-sm font-black text-[var(--text-muted)]">
            YOU ARE MY MUZE
          </p>
          <p className="mt-4 text-xs text-[var(--text-faint)]">
            No featured albums are available.
          </p>
        </div>
      </main>
    );
  ...
  const progressStyle: CSSProperties = {
    ...
    animationPlayState:
      isPageVisible && !prefersReducedMotion ? "running" : "paused",
  };
  const updateVideoStatus =
    (setStatus: Dispatch<SetStateAction<Set<string>>>) => (id: string) =>
      setStatus((current) =>
  ...
  return (
      ...
      style={
        {
          ...
          touchAction: "pan-y",
          "--slide-accent": slides[currentSlide]?.color || BRAND_PINK_HEX,
        } as CSSProperties
      }
    ...
    >
      <h1 className="sr-only">
        {slides[currentSlide]?.artistName} — {slides[currentSlide]?.title}
      </h1>
      {slides.map((slide, index) => (
        <HomeSlide
          ...
          locale={locale}
          exploreLabel={t.hero.exploreBtn}
          listenLabel={t.hero.listenBtn}
          openStreamingSlideId={openStreamingSlideId}
          readyVideoSlideIds={readyVideoSlideIds}
          failedVideoSlideIds={failedVideoSlideIds}
          shouldPreload={index <= preloadIndex}
          onStreamingToggle={(id) =>
            setOpenStreamingSlideId((current) => (current === id ? null : id))
          }
          onStreamingClose={() => setOpenStreamingSlideId(null)}
          onVideoReady={updateVideoStatus(setReadyVideoSlideIds)}
          onVideoFailure={updateVideoStatus(setFailedVideoSlideIds)}
      ...
      <HomeSlideControls
        slides={slides}
        currentSlide={currentSlide}
        autoplayElapsed={autoplayElapsed}
