...
const GALLERY_COLUMNS = "id,album_id,member_id,image_url,caption,sort_order";

function toAlbum(item: RawDiscographyAlbum): DiscographyAlbum {
  return {
    id: item.id,
    title: item.title,
    titles: {
...
export async function fetchDiscography(
  ...
  const members: DiscographyMember[] = (membersResult.data ?? []).map(
    (member) => ({
      ...
      roles: { ko: member.role_ko, en: member.role_en, ja: member.role_ja },
  ...
  return {
    ...
    albums: (albumsResult.data ?? []).map(toAlbum),
    members,
    gallery,
  };
}
