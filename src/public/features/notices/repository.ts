...
const NOTICE_LIST_COLUMNS =
  "id,title_ko,title_en,title_ja,category_ko,category_en,category_ja,date";
const NOTICE_DETAIL_COLUMNS =
  "id,title_ko,title_en,title_ja,content_ko,content_en,content_ja,category_ko,category_en,category_ja,date";
...
async function resolveNoticeScope(
...
export async function getPublicNotices(
  ...
  query = scope.artistId
    ? query.eq("artist_id", scope.artistId)
  ...
  return {
    name: scope.name,
    notices: ((data ?? []) as NoticeRow[]).map(toNoticeListItemDTO),
  };
}
...
export async function getPublicNotice(
  ...
  return {
    name: scope.name,
    notice: data ? toNoticeDTO(data as NoticeRow) : null,
  };
}
...
export async function getPublicNoticeNavigation(
  client: SupabaseClient,
  noticeId: string,
  artistSlug?: string,
): Promise<NoticeNavigationDTO> {
  const scope = await resolveNoticeScope(client, artistSlug);
  let query = client
    .from("notices")
    .select(NOTICE_NAVIGATION_COLUMNS)
    .eq("is_published", true)
    .order("published_at", { ascending: false });
  query = scope.artistId
    ? query.eq("artist_id", scope.artistId)
  ...
  const { data, error } = await query;
  if (error) throw error;
  const notices = (data ?? []) as Pick<
    NoticeRow,
    "id" | "title_ko" | "title_en" | "title_ja"
  >[];
  const index = notices.findIndex((notice) => notice.id === noticeId);
  const item = (notice: (typeof notices)[number] | undefined) =>
    notice
      ? {
          id: notice.id,
          title: {
      ...
      : null;
  return {
    previous: item(index >= 0 ? notices[index + 1] : undefined),
    next: item(index > 0 ? notices[index - 1] : undefined),
  };
}
