...
function accepts(mimeType: string, accepted: string[]) {
  return (
    ...
    accepted.some(
      (item) =>
        item === mimeType ||
        (item.endsWith("/*") && mimeType.startsWith(item.slice(0, -1))),
    )
  );
}
...
): value is Extract<AuditionAnswer, { path: string }> {
  return (
    typeof value === "object" &&
    !Array.isArray(value) &&
...
export async function POST(request: NextRequest) {
  ...
  const [campaignResult, fieldResult] = await Promise.all([
    service
      ...
      .select(
        "id,title,description,description_i18n,is_active,starts_at,ends_at,created_at,updated_at",
      )
      .eq("id", campaignId)
      .eq("is_active", true)
      .maybeSingle(),
    service
      ...
      .select(
        "id,campaign_id,field_key,label_i18n,help_text,field_type,options,required,max_length,max_file_size_mb,accepted_file_types,sort_order,is_active,is_primary_label",
      )
      .eq("campaign_id", campaignId)
      .eq("is_active", true)
      .order("sort_order"),
  ]);
  if (campaignResult.error || fieldResult.error)
  ...
  if (
    ...
    (activeCampaign.starts_at &&
      Date.parse(activeCampaign.starts_at) > Date.parse(now)) ||
    (activeCampaign.ends_at &&
      Date.parse(activeCampaign.ends_at) < Date.parse(now))
  ) {
  ...
  const { data: existing, error: existingError } = requestedSubmissionId
    ? await service
        ...
        .select(
          "id,campaign_id,answers,status,reviewer_notes,reviewed_by,reviewed_at,created_at,updated_at",
        )
  ...
  if (
    ...
    (!existing || existing.campaign_id !== campaignId)
  )
    return errorResponse("SUBMISSION_NOT_FOUND", 404);
  if (
    existing &&
    (existing.status !== "pending" ||
      existing.reviewer_notes !== null ||
  ...
  for (const field of fields) {
    if (field.field_type === "file") {
      ...
      if (!file) {
        const previous = existing?.answers?.[field.field_key] as
          AuditionAnswer | undefined;
        if (
          formData.get(`keepFiles[${field.field_key}]`) === "true" &&
          storedFile(previous)
        ) {
      ...
      if (file.size > (field.max_file_size_mb ?? 20) * 1024 * 1024)
        return errorResponse("FILE_TOO_LARGE", 413);
      const validated = await validateFileSignature(
        file,
        "audition-attachment",
      );
      if (
        !validated ||
        !extensionMatches(file.name, validated.extension) ||
        !accepts(validated.mimeType, field.accepted_file_types)
      ) {
    ...
    if (field.field_type === "checkbox") {
      const values = formData
        .getAll(`answers[${field.field_key}]`)
      ...
      if (
        ...
        return errorResponse("INVALID_OPTION", 422);
    ...
    if (
      ...
      return errorResponse("INVALID_OPTION", 422);
    if (field.field_type === "date" && value && !validDate(value))
      return errorResponse("INVALID_DATE", 422);
    if (
      (field.field_type === "short_text" || field.field_type === "long_text") &&
      value.length >
        (field.max_length ?? (field.field_type === "short_text" ? 255 : 5000))
    )
      return errorResponse("ANSWER_TOO_LONG", 422);
    answers[field.field_key] = value;
  }

  const emailField = fields.find((field) => EMAIL_KEYS.has(field.field_key));
  const email = emailField
    ? text(formData, emailField.field_key).toLowerCase()
    : "";
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254)
    return errorResponse("VALID_EMAIL_REQUIRED", 422);
  const accountEmail = user.email?.trim().toLowerCase() ?? "";
  if (!accountEmail || !user.email_confirmed_at || email !== accountEmail)
    return errorResponse("EMAIL_ACCOUNT_MISMATCH", 422);
  ...
  const emailHash = createHmac("sha256", secret)
    .update(`${campaignId}:${accountEmail}`)
    .digest("hex");
  let duplicateQuery = service
    .from("audition_submissions")
    .select("id", { count: "exact", head: true })
    .eq("campaign_id", campaignId)
    .eq("applicant_email_hash", emailHash);
  ...
  let persistedSubmission: { created_at?: string; updated_at?: string } | null =
    null;
  try {
    ...
    if (dbError.code === "P0001" && dbError.message === "CAMPAIGN_CLOSED")
      return errorResponse("CAMPAIGN_CLOSED", 404);
    if (dbError.code === "P0001" && dbError.message === "SUBMISSION_CONFLICT")
      return errorResponse("SUBMISSION_CONFLICT", 409);
  ...
  const result = NextResponse.json(
    {
      ...
      submission: {
        ...
        reviewed_at: null,
        created_at:
          existing?.created_at ?? persistedSubmission?.created_at ?? timestamp,
        updated_at: persistedSubmission?.updated_at ?? timestamp,
      },
    },
