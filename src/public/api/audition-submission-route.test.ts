...
const service = {
  from: vi.fn((table: string) => {
    if (table === "audition_campaigns")
      return chain({ data: campaign, error: null });
    if (table === "audition_form_fields")
      return chain({ data: fields, error: null });
    return {
      select: vi.fn((columns: string) =>
        columns.includes("answers")
          ? chain({ data: mocks.existing, error: null })
  ...
  it("builds the snapshot server-side and ties the service-role write to the signed-in user", async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        p_applicant_email_hash: expect.any(String),
      }),
    );
  });
  ...
  it("updates only the signed-in user's existing submission while the campaign is open", async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        p_user_id: "user-1",
        p_campaign_id: campaign.id,
        p_answers: expect.objectContaining({ part: "댄스" }),
      }),
    );
  });
