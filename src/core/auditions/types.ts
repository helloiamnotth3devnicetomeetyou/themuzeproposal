...
export type AuditionFieldType = "short_text" | "long_text" | "select" | "radio" | "checkbox" | "date" | "file" | "consent";
...
export type AuditionAnswer = string | string[] | { path: string; name: string; size: number; mimeType: string };
...
export function isCampaignOpen(campaign: Pick<AuditionCampaign, "is_active" | "starts_at" | "ends_at">, now = Date.now()) {
...
export function fieldLabel(field: Pick<AuditionFormField, "field_key" | "label_i18n">, locale: string) {
  return field.label_i18n[locale as keyof LocalizedLabel]?.trim()
...
export function campaignDescription(campaign: Pick<AuditionCampaign, "description" | "description_i18n">, locale: string) {
  return campaign.description_i18n?.[locale as keyof LocalizedLabel]?.trim()
