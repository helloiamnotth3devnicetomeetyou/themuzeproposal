...
describe("GET /api/asset-proxy", () => {
  ...
  it.each([
    ["svg", "artist/album-typography-sanitized/album/123e4567-e89b-12d3-a456-426614174000.svg", "image/png"],
    ["jpg", "artist/profile/image.jpg", "image/webp"],
    ["jpeg", "artist/profile/image.jpeg", "image/png"],
    ["png", "artist/profile/image.png", "image/jpeg"],
    ["webp", "artist/profile/image.webp", undefined],
  ])("rejects %s assets with missing or mismatched upstream MIME", async (_, path, contentType) => {
    vi.stubEnv("NEXT_PUBLIC_R2_PUBLIC_URL", "https://cdn.example.com");
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
    ...
    const url = `https://cdn.example.com/artist-assets/${path}`;
    const response = await GET(
      new NextRequest(
        `https://themuze.kr/api/asset-proxy?url=${encodeURIComponent(url)}`,
      ),
    ...
    await expect(response.json()).resolves.toEqual({ code: "INVALID_ASSET" });
  });
});
