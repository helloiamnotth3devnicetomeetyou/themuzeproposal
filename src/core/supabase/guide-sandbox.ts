...
export const isGuideSandboxActive = () =>
  browserStorage()?.getItem(ACTIVE_KEY) === "true";
...
export function finishGuideSandbox() {
  ...
  try {
    snapshot = JSON.parse(storage.getItem(DRAFT_SNAPSHOT_KEY) ?? "{}");
  } catch {
    /* discard an invalid sandbox snapshot */
  }
  Object.keys(storage)
    .filter((key) => key.startsWith(DRAFT_PREFIX))
...
export function createGuideSandboxAsset<Bucket extends string>(
  file: Blob,
  bucket: Bucket,
  path: string,
) {
...
export function isGuideSandboxWrite(url: string, method: string) {
  if (!isGuideSandboxActive() || method === "GET" || method === "HEAD")
    return false;
...
async function sandboxResponse(request: Request) {
  ...
  if (url.pathname.startsWith("/api/uploads/")) {
    ...
    if (file instanceof Blob) {
      ...
      return Response.json({
        asset: createGuideSandboxAsset(file, bucket, path),
        token: "guide-sandbox",
      });
    }
  }
  ...
  if (request.method === "POST" || request.method === "PATCH") {
    const id =
      url.searchParams.get("id")?.replace(/^eq\./, "") || crypto.randomUUID();
    const rows = Array.isArray(payload)
      ? payload
      : [{ id, ...(payload ?? {}) }];
    const body = request.headers.get("accept")?.includes("vnd.pgrst.object")
      ? rows[0]
      : rows;
...
export async function guideSandboxFetch(
  ...
  if (!isGuideSandboxWrite(request.url, request.method))
    return fetch(input, init);
  if (typeof window !== "undefined" && Date.now() - lastNoticeAt > 800) {
    lastNoticeAt = Date.now();
    window.dispatchEvent(
      new CustomEvent("admin-toast", {
        detail: "연습 모드라서 실제 데이터에는 반영하지 않았습니다.",
      }),
    );
  }
