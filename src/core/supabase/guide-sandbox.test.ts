...
import { finishGuideSandbox, guideSandboxFetch, isGuideSandboxActive, isGuideSandboxWrite, startGuideSandbox } from "./guide-sandbox";
...
describe("admin guide sandbox", () => {
  ...
  it("blocks content writes but keeps reads and guide progress", () => {
    ...
    expect(isGuideSandboxWrite("https://example.supabase.co/rest/v1/artists", "PATCH")).toBe(true);
    expect(isGuideSandboxWrite("https://example.supabase.co/rest/v1/artists", "GET")).toBe(false);
    expect(isGuideSandboxWrite("https://example.supabase.co/rest/v1/admin_onboarding_progress", "POST")).toBe(false);
    expect(isGuideSandboxWrite("https://example.supabase.co/storage/v1/object/sign/protect-evidence/file.webp", "POST")).toBe(false);
  });
  ...
  it("returns a local representation for a blocked database update", async () => {
    startGuideSandbox();
    const response = await guideSandboxFetch("https://example.supabase.co/rest/v1/artists?id=eq.artist-1", {
      method: "PATCH",
