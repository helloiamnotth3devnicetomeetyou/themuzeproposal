...
describe("admin roles", () => {
  it.each([
    ...
    await expect(getAdminRole(query.client as never, "user-1")).resolves.toBe(expected);
    expect(query.select).toHaveBeenCalledWith("role");
    expect(query.eq).toHaveBeenCalledWith("id", "user-1");
  });

  it("fails closed on database errors", async () => {
    const query = client({ data: { role: "super_admin" }, error: new Error("database unavailable") });
  ...
  it("distinguishes super admins from editors", async () => {
    ...
    await expect(isSuperAdmin(query.client as never, "user-1")).resolves.toBe(false);
  });
});
