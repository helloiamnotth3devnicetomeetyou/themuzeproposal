...
function snapshotLocaleText() {
  ...
  while (walker.nextNode()) {
    ...
    if (node.parentElement) snapshot.set(node.parentElement, [...(snapshot.get(node.parentElement) ?? []), node.data]);
  }
  return snapshot;
}
...
function animateLocaleText(previousText: Map<Element, string[]>) {
  ...
  while (walker.nextNode()) {
    ...
    if (previousText.get(parent)?.[position] !== node.data && node.data.trim() && !parent.childElementCount && rect.bottom >= 0 && rect.top <= innerHeight && !parent.closest("script, style, [aria-hidden='true']")) nodes.push({ node, parent });
  }
  ...
  nodes
    ...
    .forEach(({ node, parent }) => {
      const letters = [...node.data].filter((letter) => !/\s/.test(letter)).length;
      const animation = parent.animate?.([
        { opacity: 0, filter: "blur(10px)", clipPath: "inset(0 100% 0 0)", transform: "translateX(-.35em)" },
        { opacity: 1, filter: "none", clipPath: "inset(0)", transform: "none" },
      ], { duration: Math.max(160, letters * 21), delay, easing: `steps(${Math.max(letters, 1)}, end)`, fill: "backwards" });
