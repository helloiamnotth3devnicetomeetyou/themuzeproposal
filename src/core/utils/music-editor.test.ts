...
describe("music editor utilities", () => {
  it("parses numbered bulk tracks while preserving their titles", () => {
    ...
    expect(parseBulkTracks("01. First\n2) Second\n\n").map(({ id, title }) => ({ id, title }))).toEqual([
      { id: "one", title: "First" }, { id: "two", title: "Second" },
    ]);
  });

  it("separates save and publish requirements", () => {
    expect(validateAlbum(draft({ title: "", release_date: "", cover_url: "", tracks: [] }))).toMatchObject({
      canSave: false, canPublish: false,
      saveIssues: expect.arrayContaining([expect.any(String)]),
      publishIssues: expect.arrayContaining([expect.any(String)]),
    });
    expect(validateAlbum(draft())).toMatchObject({ canSave: true, canPublish: true });
  });

  it("only recognizes managed asset storage URLs", () => {
    expect(managedAssetFromUrl("https://project.supabase.co/storage/v1/object/public/track-assets/a%20b/file.mp3"))
