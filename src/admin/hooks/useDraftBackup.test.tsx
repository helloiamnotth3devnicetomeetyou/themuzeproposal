...
describe("useDraftBackup", () => {
  ...
  it("restores the backup and announces it through the admin toast", async () => {
    localStorage.setItem(
      "draft",
      JSON.stringify({ draft: { title: "복구본" }, updatedAt: 1 }),
    );
    const restore = vi.fn();
    const onToast = vi.fn();
    ...
    const { result } = renderHook(() =>
      useDraftBackup({
        key: "draft",
        draft: { title: "서버본" },
        snapshot: JSON.stringify({ title: "서버본" }),
        dirty: false,
        restore,
      }),
    );
    ...
    expect((onToast.mock.calls[0][0] as CustomEvent<string>).detail).toBe(
      "임시 작업을 복구했습니다.",
    );
  ...
  it("does not offer sandbox drafts for recovery", async () => {
    ...
    localStorage.setItem(
      "draft",
      JSON.stringify({ draft: { title: "연습본" }, updatedAt: 1 }),
    );
    const { result } = renderHook(() =>
      useDraftBackup({
        key: "draft",
