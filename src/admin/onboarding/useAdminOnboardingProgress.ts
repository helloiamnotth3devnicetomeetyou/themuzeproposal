...
const missingTable = (message?: string) =>
  Boolean(
    message && /does not exist|schema cache|could not find/i.test(message),
  );

export function useAdminOnboardingProgress({
  userId,
  role,
}: {
  userId?: string;
  ...
  const [progressRows, setProgressRows] = useState<
    Record<string, GuideProgressRow>
  >({});
  const [capabilities, setCapabilities] = useState<GuideCapabilities>({
    artistScenes: true,
    artistGallery: true,
  });
  ...
  useEffect(() => {
    ...
    void Promise.all([
      supabase
        ...
        .select("chapter_id,furthest_step_id,completed_at"),
      supabase
        .from("artist_scenes")
      ...
      const nextRows = Object.fromEntries(
        rows.map((row) => [row.chapter_id, row]),
      );
  ...
  const saveStepProgress = async (
    ...
    const currentIndex = available.findIndex(
      (item) => item.id === current?.furthest_step_id,
    );
