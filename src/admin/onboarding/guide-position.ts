export type GuideRect = { top: number; left: number; width: number; height: number };
export type GuidePlacement = "right" | "left" | "below" | "above";
export type GuidePosition = { top: number; left: number; placement: GuidePlacement };
...
export function getGuideHighlightRect(target: GuideRect, viewport: { width: number; height: number }): GuideRect {
  ...
  const right = clamp(target.left + target.width + HIGHLIGHT_PADDING, left, viewport.width - 8);
  const bottom = clamp(target.top + target.height + HIGHLIGHT_PADDING, top, viewport.height - 8);
...
export function getGuidePosition(
  ...
  const right = viewport.width - (target.left + target.width) - TARGET_GAP - VIEWPORT_GUTTER;
  const left = target.left - TARGET_GAP - VIEWPORT_GUTTER;
  const below = viewport.height - (target.top + target.height) - TARGET_GAP - VIEWPORT_GUTTER;
  const above = target.top - TARGET_GAP - VIEWPORT_GUTTER;
  const spaces = { right, left, below, above };
  const placement = (["right", "left", "below", "above"] as const).find((side) =>
    spaces[side] >= (side === "right" || side === "left" ? popover.width : popover.height),
  ) ?? (["right", "left", "below", "above"] as const).reduce((best, side) => spaces[side] > spaces[best] ? side : best);
  ...
  const raw = placement === "right"
    ...
    : placement === "left"
      ...
      : placement === "below"
        ...
        : { top: target.top - popover.height - TARGET_GAP, left: centeredLeft };

  return {
    placement,
    top: clamp(raw.top, VIEWPORT_GUTTER, viewport.height - popover.height - VIEWPORT_GUTTER),
    left: clamp(raw.left, VIEWPORT_GUTTER, viewport.width - popover.width - VIEWPORT_GUTTER),
  };
}
