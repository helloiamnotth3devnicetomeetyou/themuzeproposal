...
export function useGuideTargetTracking({
  ...
  useEffect(() => {
    if (!step || !guidePathMatches(step.href.replace(":artistId", artistId ?? "new"), pathname, step.descendantPath)) return;
    ...
    const usableViewport = () => {
      ...
      return { width: window.innerWidth, height: Math.max(160, visualHeight - sheetHeight - (mobile ? 12 : 0)), visualHeight };
    };
    ...
    const isExposed = (element: HTMLElement) => {
      ...
      return [[bounds.left + bounds.width / 2, bounds.top + bounds.height / 2], [bounds.left + insetX, bounds.top + insetY], [bounds.right - insetX, bounds.bottom - insetY]].some(([x, y]) => {
    ...
    const visibleTarget = (tourId: string) => {
      ...
      return exposed.find((candidate) => !candidate.matches(":disabled")) ?? candidates.find((candidate) => !candidate.matches(":disabled")) ?? exposed[0] ?? candidates[0] ?? null;
    };
    const update = () => {
      if (!target) return;
      ...
      setPopoverPosition(getGuidePosition(targetRect, { width: popover?.width ?? Math.min(380, viewport.width - 32), height: popover?.height ?? 360 }, viewport));
    };
    const scheduleUpdate = () => { cancelAnimationFrame(animationFrame); animationFrame = requestAnimationFrame(update); };
    const observeTarget = () => {
    ...
    const revealTarget = (element: HTMLElement, force = false) => {
      ...
      revealTimer = window.setTimeout(() => {
        ...
        const delta = next.bottom > nextViewport.height - 20 ? next.bottom - nextViewport.height + 20 : next.top < 20 ? next.top - 20 : 0;
        if (delta) (getScrollableParent(element) ?? window).scrollBy({ top: delta, behavior: "smooth" });
        scheduleUpdate();
      }, 360);
    };
    ...
    const completePractice = (event: Event) => {
      ...
      if (step.practice?.event === "input" || step.practice?.event === "change") {
        ...
        if ((field instanceof HTMLInputElement || field instanceof HTMLTextAreaElement || field instanceof HTMLSelectElement) && !field.value.trim()) return;
    ...
    const attach = (nextTarget: HTMLElement, prompt: string | null = null, tourId = "") => {
      ...
      promptFrame = requestAnimationFrame(() => { setInteractionPrompt(prompt); if (prompt && window.matchMedia("(max-width: 700px)").matches) setMobileSheetExpanded(false); });
      armPractice(tourId);
    ...
    const findPrimary = () => { const nextTarget = visibleTarget(step.target); return nextTarget ? attach(nextTarget, null, step.target) : false; };
    const findInteraction = () => { const nextTarget = step.interaction ? visibleTarget(step.interaction.target) : null; return nextTarget && step.interaction ? attach(nextTarget, step.interaction.instruction, step.interaction.target) : false; };
    const skipMissingStep = () => {
      if (skipped) return;
      skipped = true;
      target?.classList.remove("admin-guide-target");
      setRect(null); setPopoverPosition(null); setInteractionPrompt(null); setIsExploring(false);
    ...
    const revealActiveTarget = () => { if (target) revealTarget(target, true); };
    window.addEventListener("admin-guide-reveal-target", revealActiveTarget);
    window.visualViewport?.addEventListener("resize", scheduleUpdate);
    window.visualViewport?.addEventListener("scroll", scheduleUpdate);
    return () => {
      observer.disconnect(); target?.classList.remove("admin-guide-target");
      if (practiceArmed && step.practice) document.removeEventListener(step.practice.event, completePractice, true);
      resizeObserver?.disconnect(); cancelAnimationFrame(animationFrame); cancelAnimationFrame(promptFrame);
      window.clearTimeout(navigationTimer); window.clearTimeout(searchTimer); window.clearTimeout(lostTimer); window.clearTimeout(revealTimer); window.clearTimeout(practiceTimer);
      window.removeEventListener("resize", scheduleUpdate); window.removeEventListener("scroll", scheduleUpdate, true); window.removeEventListener("admin-guide-reveal-target", revealActiveTarget);
      window.visualViewport?.removeEventListener("resize", scheduleUpdate); window.visualViewport?.removeEventListener("scroll", scheduleUpdate);
    };
