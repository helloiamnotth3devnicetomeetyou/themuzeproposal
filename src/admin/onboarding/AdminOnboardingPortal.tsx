...
export type AdminOnboardingPortalProps = {
  mounted: boolean;
  welcomeOpen: boolean;
  tocOpen: boolean;
  chapterIntro: GuideRun | null;
  introChapter?: GuideChapter;
  run: GuideRun | null;
  step?: GuideStep;
  dialogRef: RefObject<HTMLDivElement | null>;
  trapFocus: (event: KeyboardEvent<HTMLDivElement>) => void;
  closeGuide: () => void;
  chooseWelcome: (mode: "full" | "toc") => Promise<void>;
  reachedSteps: number;
  totalSteps: number;
  progress: number;
  chapterStats: Record<string, Stats>;
  completed: Set<string>;
  startChapter: (id: string) => void;
  chapterSteps: Record<string, GuideStep[]>;
  openStep: (chapterId: string, index: number, mode: GuideRun["mode"]) => boolean;
  isExploring: boolean;
  isMobileGuide: boolean;
  mobileSheetExpanded: boolean;
  visibleRect: Rect | null;
  activePopoverPosition: ActivePopoverPosition | null;
  popoverPosition: GuidePosition | null;
  isPopoverDragging: boolean;
  interactionPrompt: string | null;
  practiceComplete: boolean;
  runChapter?: GuideChapter;
  steps: GuideStep[];
  mobileSheetDragRef: RefObject<number | null>;
  toggleMobileSheet: () => void;
  startMobileSheetDrag: (event: PointerEvent<HTMLButtonElement>) => void;
  stopMobileSheetDrag: (event: PointerEvent<HTMLButtonElement>) => void;
  startPopoverDrag: (event: PointerEvent<HTMLElement>) => void;
  movePopover: (event: PointerEvent<HTMLElement>) => void;
  stopPopoverDrag: (event: PointerEvent<HTMLElement>) => void;
  updateMobileSheet: (expanded: boolean) => void;
  setIsExploring: (value: boolean) => void;
  setChapterIntro: (value: GuideRun | null) => void;
  setTocOpen: (value: boolean) => void;
  setRun: (value: GuideRun | null) => void;
  setRect: (value: Rect | null) => void;
  setPausedRun: (value: GuideRun | null) => void;
  setPopoverPosition: (value: GuidePosition | null) => void;
  finishOrAdvance: () => Promise<void>;
};

export function AdminOnboardingPortal(props: AdminOnboardingPortalProps) {
  const {
    mounted, welcomeOpen, tocOpen, chapterIntro, introChapter, run, step,
    dialogRef, trapFocus, closeGuide, chooseWelcome, reachedSteps, totalSteps,
    progress, chapterStats, completed, startChapter, chapterSteps, openStep,
    isExploring, isMobileGuide, mobileSheetExpanded, visibleRect,
    activePopoverPosition, popoverPosition, isPopoverDragging, interactionPrompt,
    practiceComplete, runChapter, steps, mobileSheetDragRef, toggleMobileSheet,
    startMobileSheetDrag, stopMobileSheetDrag, startPopoverDrag, movePopover,
    stopPopoverDrag, updateMobileSheet, setIsExploring, setChapterIntro,
    setTocOpen, setRun, setRect, setPausedRun, setPopoverPosition, finishOrAdvance,
  } = props;
  ...
  return createPortal(
    <>
      {welcomeOpen && (
        <div className="admin-guide-modal-backdrop">
          <div ref={dialogRef} className="admin-guide-welcome" role="dialog" aria-modal="true" aria-labelledby="admin-guide-welcome-title" onKeyDown={trapFocus}>
            ...
            <p>실제 데이터를 바꾸지 않고 모든 업무 버튼의 용도와 주의사항을 바로 익힐 수 있습니다.</p>
      ...
      {tocOpen && (
        <div className="admin-guide-modal-backdrop admin-guide-toc-backdrop">
          <aside ref={dialogRef} className="admin-guide-toc" role="dialog" aria-modal="true" aria-labelledby="admin-guide-toc-title" onKeyDown={trapFocus}>
            ...
            <nav aria-label="가이드 목차">
              {GUIDE_CHAPTERS.map((chapter, index) => {
                ...
                const percent = stats.total ? Math.round((stats.reached / stats.total) * 100) : 0;
                return <button type="button" key={chapter.id} style={{ "--guide-chapter-delay": `${110 + index * 34}ms` } as CSSProperties} className={completed.has(chapter.id) ? "is-complete" : ""} onClick={() => startChapter(chapter.id)}><span className="admin-guide-chapter-number">{chapter.id.padStart(2, "0")}</span><span><b>{chapter.title}</b><em><i style={{ width: `${percent}%` }} />{stats.reached}/{stats.total}</em></span><i>{completed.has(chapter.id) ? <Check aria-label="완료" /> : <ChevronRight aria-hidden="true" />}</i></button>;
              })}
            </nav>
          </aside>
        </div>
      )}
      {chapterIntro && introChapter && (
        <div className="admin-guide-modal-backdrop admin-guide-chapter-intro-backdrop">
          <section ref={dialogRef} className="admin-guide-chapter-intro" role="dialog" aria-modal="true" aria-labelledby="admin-guide-chapter-intro-title" onKeyDown={trapFocus}>
            ...
            <span>챕터 {introChapter.id} · {chapterSteps[introChapter.id]?.length ?? 0}개 기능</span><h2 id="admin-guide-chapter-intro-title">{introChapter.title}</h2><p>{introChapter.description}</p>
      ...
      {run && step && (
        <div className={`admin-guide-layer${isExploring ? " is-exploring" : ""}${isMobileGuide && !mobileSheetExpanded ? " is-sheet-collapsed" : ""}`} aria-live="polite">
