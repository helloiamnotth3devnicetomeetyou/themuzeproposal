...
describe("admin guide positioning", () => {
  ...
  it("places the guide beside the target when space is available", () => {
    expect(getGuidePosition({ top: 200, left: 200, width: 160, height: 60 }, popover, viewport)).toMatchObject({ placement: "right", left: 374 });
    expect(getGuidePosition({ top: 200, left: 1000, width: 120, height: 60 }, popover, viewport)).toMatchObject({ placement: "left", left: 626 });
  });

  it("uses vertical space when neither side is wide enough", () => {
    expect(getGuidePosition({ top: 120, left: 340, width: 220, height: 60 }, popover, { width: 900, height: 900 }).placement).toBe("below");
    expect(getGuidePosition({ top: 720, left: 340, width: 220, height: 60 }, popover, { width: 900, height: 900 }).placement).toBe("above");
  });

  it("keeps highlights and guide cards inside the viewport", () => {
    expect(getGuideHighlightRect({ top: -20, left: -40, width: 200, height: 100 }, viewport)).toEqual({ top: 8, left: 8, width: 160, height: 80 });
    const position = getGuidePosition({ top: 760, left: 1160, width: 80, height: 80 }, popover, viewport);
    ...
    const compact = getGuidePosition({ top: 220, left: 120, width: 80, height: 40 }, { width: 288, height: 420 }, { width: 320, height: 480 });
