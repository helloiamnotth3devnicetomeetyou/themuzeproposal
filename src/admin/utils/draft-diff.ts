...
const itemKey = (value: unknown) => {
  ...
  return String(
    item.id ?? item.key ?? item.slug ?? item.url ?? JSON.stringify(value),
  );
};
...
export function buildDraftDiff(
  ...
  if (Array.isArray(before) && Array.isArray(after)) {
    ...
    const sameItems =
      ...
      [...beforeKeys].sort().join("\0") === [...afterKeys].sort().join("\0");
    const kind: DraftDiffKind =
      sameItems && beforeKeys.join("\0") !== afterKeys.join("\0")
        ? "order"
  ...
  return [...new Set([...Object.keys(previous), ...Object.keys(next)])].flatMap(
    (field) => {
      ...
      let kind: DraftDiffKind = empty(from)
        ? "add"
        : empty(to)
          ? "delete"
          : "change";
      if (Array.isArray(from) && Array.isArray(to)) {
        ...
        if (
          ...
          [...fromKeys].sort().join("\0") === [...toKeys].sort().join("\0") &&
          fromKeys.join("\0") !== toKeys.join("\0")
        )
          kind = "order";
      ...
      return [
        {
          kind,
          field: labels[field] || DEFAULT_FIELD_LABELS[field] || field,
          before: display(from),
          after: display(to),
        },
      ];
...
export function formatDraftPeek(items: DraftDiffItem[], limit = 6) {
  ...
  if (items.length > limit)
    visible.push(`• 그 외 ${items.length - limit}개 변경`);
  return visible.join("\n");
}
