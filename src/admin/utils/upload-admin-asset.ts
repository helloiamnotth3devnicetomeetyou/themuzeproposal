...
export async function uploadAdminAsset<Bucket extends AdminAssetBucket>(
  ...
  if (isGuideSandboxActive())
    return createGuideSandboxAsset(file, bucket, path);
  if (bucket === "hero-videos")
    return uploadHeroVideo(file, options) as Promise<{
      bucket: Bucket;
      path: string;
      url: string;
    }>;
  ...
  formData.set(
    "file",
    file,
    file instanceof File ? file.name : path.split("/").pop() || "asset",
  );
  const fileSize = file.size;
  const payload = options.onProgress
    ? await uploadWithProgress(formData, options)
    : await fetch("/api/uploads/admin-asset", {
        method: "POST",
        body: formData,
  ...
  if (!payload.ok || !body.asset) {
    const errorCode =
      body.code ||
      (payload.status === 413 ? "FILE_TOO_LARGE" : `HTTP_${payload.status}`);
    ...
    if (payload.status === 413 || errorCode === "FILE_TOO_LARGE") {
      throw new Error(
        "파일 용량이 서버 제한을 초과했습니다 (HTTP 413 / FILE_TOO_LARGE). 20MB 이하의 영상을 선택하거나 서버/Nginx 용량 제한을 확인하세요.",
      );
    }
...
async function uploadHeroVideo(file: Blob, options: UploadOptions) {
  ...
  const preparation = (await prepared.json().catch(() => ({}))) as {
    upload?: { url: string; path: string };
    code?: string;
  };
  if (!prepared.ok || !preparation.upload)
    throw new Error(preparation.code || `HTTP_${prepared.status}`);

  try {
    const uploaded = await putWithProgress(
      preparation.upload.url,
      file,
      contentType,
      options,
    );
    ...
    if (!completed.ok || !result.asset)
      throw new Error(result.code || `HTTP_${completed.status}`);
    return result.asset;
  } catch (error) {
...
function putWithProgress(
  ...
  return new Promise<{ ok: boolean; status: number }>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(
        new Error(
          "R2 직접 업로드 연결에 실패했습니다. R2 버킷 CORS에 현재 사이트 origin과 PUT/Content-Type을 허용했는지 확인하세요.",
        ),
      );
    request.onabort = () =>
...
function uploadWithProgress(
  ...
  return new Promise<{ ok: boolean; status: number; payload: unknown }>(
    (resolve, reject) => {
      const request = new XMLHttpRequest();
