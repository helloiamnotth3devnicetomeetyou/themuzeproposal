...
export function useDiscographyEditor({
  routeArtistId,
  requestConfirm,
}: {
  ...
  const validation = useMemo(
    () => (draft ? validateAlbum(draft) : null),
    [draft],
  );
  const nestedDrafts = usePageDrafts();
  const previewPayload = useMemo(
    () =>
      draft && artistId && artistSlug
        ? {
            artist: { id: artistId, slug: artistSlug, name: artistName },
            album: draft,
          }
        : null,
    [artistId, artistName, artistSlug, draft],
  );
  const { openPreview } = useAdminPreview({
    kind: "album",
    payload: previewPayload,
    targetPath: previewPayload
      ? `/${artistSlug}/discography?album=${encodeURIComponent(previewPayload.album.id)}`
      : "",
  ...
  const loadAlbums = useCallback(
    async (preferredId?: string) => {
      ...
      const albumResult = await supabase
        ...
        .overrideTypes<RawAlbum[], { merge: false }>();
      let albumRows = albumResult.data;
      let albumError = albumResult.error;
      if (
        albumError?.message.includes("typo_logo_url") ||
        albumError?.message.includes("title_ko")
      ) {
        const legacyResult = await supabase
          .from("albums")
          .select(legacyAlbumSelect)
          .eq("artist_id", artist.id)
          .order("sort_order", { ascending: true });
        albumRows = legacyResult.data
          ? (legacyResult.data.map((album) => ({
              ...album,
              typo_logo_url: null,
      ...
      const params = new URLSearchParams(window.location.search);
      const requestedId =
        preferredId || params.get("album") || nextAlbums[0]?.id;
      const selected =
        nextAlbums.find((album) => album.id === requestedId) ??
        nextAlbums[0] ??
        null;
  ...
  const patchTrack = (id: string, patch: Partial<TrackDraft>) =>
    setDraft((current) =>
      current
        ? {
            ...current,
  ...
  const removeAssets = async (assets: UploadedAsset[]) => {
    ...
    assets.forEach((asset) =>
      groups.set(asset.bucket, [
        ...(groups.get(asset.bucket) ?? []),
        asset.path,
      ]),
    );
    await Promise.all(
      [...groups].map(([bucket, paths]) => deleteAdminAssets(bucket, paths)),
    );
  };
  ...
  const selectAlbum = async (album: AlbumEditorDraft) => {
    if (
      ...
      !(await requestConfirm({
        title: "변경사항을 버릴까요?",
        description:
          "현재 앨범에서 저장하지 않은 정보와 업로드 대기 파일이 사라집니다. 다른 앨범을 열기 전에 한 번 더 확인해 주세요.",
        confirmLabel: "버리고 열기",
        tone: "danger",
      }))
    )
  ...
  const addAlbum = async () => {
    ...
    if (
      ...
      !(await requestConfirm({
        title: "새 앨범을 만들까요?",
        description:
          "현재 앨범에서 저장하지 않은 정보와 업로드 대기 파일이 사라지고 새 앨범 작성 화면으로 이동합니다.",
  ...
  const changeTab = (next: EditorTab) => {
    if (draft) {
      setTab(next);
      syncUrl(draft.id, next);
    }
  };
  const handleTitle = (title: string) =>
    patchDraft({ title, title_ko: draft?.title_ko || title });
  const registerUpload = (asset: UploadedAsset) => {
    uploadedAssets.current.push(asset);
  };

  const save = async () => {
    if (!draft || !validation?.canSave) {
      setError(
        `저장 전 확인: ${validation?.saveIssues.join(", ") || "필수 정보를 확인해 주세요."}`,
      );
      return;
    }
    if (draft.is_published && !validation.canPublish) {
      setError(`공개 전 확인: ${validation.publishIssues.join(", ")}`);
      setTab("publish");
      return;
    }
    setSaving(true);
    ...
    const stale = original
      ? [...collectAssetUrls(original)]
          ...
          .map(managedAssetFromUrl)
          .filter(Boolean)
      : [];
    await Promise.all(
      stale.map((asset) => deleteAdminAssets(asset!.bucket, [asset!.path])),
    );
    uploadedAssets.current = [];
    setSaving(false);
  ...
  const removeAlbum = async () => {
    ...
    setDeleting(true);
    const assets = [...collectAssetUrls(draft)]
      .map(managedAssetFromUrl)
      .filter(Boolean);
    ...
    await Promise.all(
      assets.map((asset) => deleteAdminAssets(asset!.bucket, [asset!.path])),
    );
    setDeleting(false);
    setDeleteOpen(false);
    setToast("앨범을 삭제했습니다.");
  ...
  return {
    ...
    reorderTrack,
    applyBulk,
    visibleAlbums: filterAlbums(albums, search, filter),
  };
}
