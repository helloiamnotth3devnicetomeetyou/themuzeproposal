...
vi.mock("@/admin/components/shell/AdminDialogProvider", () => ({
  useAdminConfirm: () => vi.fn().mockResolvedValue(true),
}));
vi.mock("@/admin/components/shell/DeleteConfirmDialog", () => ({
  default: () => null,
}));
vi.mock("@/admin/components/assets/MusicAssetFields", () => ({
  CoverAssetField: () => null,
  HeroAssetField: () => null,
  TrackAssetField: () => null,
}));
vi.mock("@/admin/components/assets/GalleryManager", () => ({
  default: () => null,
}));
vi.mock("@/admin/components/content/PreviewButton", () => ({
  default: () => null,
}));
vi.mock("@/admin/components/shell/AdminSkeleton", () => ({
  default: () => null,
}));
vi.mock("@/core/components/form/CustomSelect", () => ({
  default: () => <select aria-label="album type" />,
}));
vi.mock("@/admin/hooks/useAdminPreview", () => ({
  useAdminPreview: () => ({ openPreview: vi.fn(), canPreview: true }),
}));
      ...
      table === "artists"
        ? {
            select: () => ({
              eq: () => ({
                maybeSingle: async () => ({
                  data: { id: "artist-1", name: "Artist", slug: "artist" },
                  error: null,
                }),
              }),
            }),
...
describe("DiscographyAdmin", () => {
  it("renders the loaded draft and sends a save request", async () => {
    ...
    const save = container.querySelector(
      ".music-header-actions .admin-btn-primary",
    )!;
