...
export default function ArtistScheduleAdminPage() {
  ...
  const rail = <>
    ...
    <div className="content-library-list">
      ...
      {monthItems.map((item) => { const date = new Date(`${item.event_date}T00:00:00`); return <button key={item.id} type="button" data-tour-id="entity-list-item" onClick={() => select(item)} className={`content-library-item ${draft?.id === item.id ? "is-selected" : ""} ${styles.railItem}`}><span className={styles.railDate}><b>{String(date.getDate()).padStart(2, "0")}</b><small>{date.toLocaleString("en", { month: "short" }).toUpperCase()}</small></span><span className="content-library-copy"><b>{item.title_ko}</b><small>{CATEGORY[item.category].label}{item.start_time ? ` · ${item.start_time.slice(0, 5)}` : ""}</small></span><i className={`content-library-dot ${item.is_published ? "is-live" : ""}`} /></button>; })}
