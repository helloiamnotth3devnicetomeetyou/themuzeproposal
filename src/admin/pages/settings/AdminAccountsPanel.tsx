...
const errorMessages: Record<string, string> = {
  ...
  CANNOT_CHANGE_OWN_ROLE:
    "현재 로그인한 계정의 역할은 여기서 변경할 수 없습니다.",
  LAST_SUPER_ADMIN:
    "마지막 슈퍼 관리자는 해제하거나 역할을 변경할 수 없습니다.",
...
export default function AdminAccountsPanel({ onError, onSuccess }: Props) {
  ...
  const remove = async (account: AdminAccount) => {
    if (!window.confirm(`${account.email} 계정의 관리자 권한을 해제할까요?`))
      return;
    setBusyId(account.id);
    const response = await guideSandboxFetch("/api/admin/accounts", {
      method: "DELETE",
    ...
    <div className="admin-accounts-stack">
      <div className="content-section-heading settings-section-heading">
        <div>
          <h3>관리자 계정</h3>
          <p>
            슈퍼 관리자는 관리자 계정을 초대하고, 역할을 변경하거나 권한을
            해제할 수 있습니다.
          </p>
        </div>
      ...
      <section
        className="admin-accounts-invite"
        data-tour-id="admin-account-invite"
      >
        <div className="admin-accounts-section-title">
          <span>
          ...
          <div>
            <b>관리자 초대 또는 승격</b>
            <small>
              기존 회원 이메일을 입력하면 초대 대신 역할만 부여합니다.
            </small>
          </div>
        </div>
      ...
      <section className="admin-accounts-list" aria-label="관리자 계정 목록">
        <div className="admin-accounts-list-head">
          <div>
            ...
            <small>
              {loading ? (
                <span
                  className="admin-skeleton-line admin-accounts-count-skeleton"
                  aria-label="불러오는 중"
                />
              ) : (
                `${accounts.length}명`
        ...
        {loading && (
          <AdminSkeleton variant="table" className="min-h-[150px]" rows={3} />
        )}
        {accounts.map((account) => {
          ...
          return (
            <article className="admin-account-row" key={account.id}>
              <span
                className={`admin-account-badge ${account.role}`}
                aria-label={
                  account.role === "super_admin" ? "슈퍼 관리자" : "편집자"
                }
              >
              ...
              <label className="admin-account-role">
                <span className="sr-only">역할</span>
                <select
                  className="admin-input"
                  value={account.role}
                  disabled={busy}
                  onChange={(event) =>
                    void changeRole(account, event.target.value as AdminRole)
                  }
                >
                  <option value="editor">편집자</option>
                  <option value="super_admin">슈퍼 관리자</option>
              ...
              <button
                className="admin-account-remove"
                data-tour-id="admin-account-remove"
                type="button"
                disabled={busy}
                onClick={() => void remove(account)}
                aria-label={`${account.email} 관리자 권한 해제`}
                title="관리자 권한 해제"
              >
