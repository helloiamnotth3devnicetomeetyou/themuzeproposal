...
import { ArrowDown, ArrowUp, Crop, ImagePlus, Trash2, Upload } from "lucide-react";
...
const serialize = (items: AvatarAsset[]) => JSON.stringify(items.map((item, index) => ({
  ...item,
  sort_order: index + 1,
})));

export default function AvatarAssetManager({ active, onDirtyChange, onError, onToast }: {
  active: boolean;
  ...
  const restoreItems = useCallback((saved: AvatarAsset[]) => {
    ...
    uploadedAssets.current = saved.filter((item) => !persistedPaths.has(item.image_path)).map((item) => ({
  ...
  const { recovery, restoreBackup, discardBackup } = useDraftBackup({ key: backupKey, draft: items, snapshot: serialize(snapshot), dirty, restore: restoreItems });
  ...
  const loadAssets = useCallback(async (nextArtistId: string) => {
    if (!nextArtistId) {
      setItems([]);
      setSnapshot([]);
      return;
  ...
  useEffect(() => {
    let mounted = true;
    void supabase.from("artists").select("id,name,eng_name").order("name", { ascending: true }).then(({ data, error }) => {
      if (!mounted) return;
      if (error) {
        setLoading(false);
  ...
  const selectArtist = async (nextArtistId: string) => {
    ...
    if (dirty && !window.confirm("저장하지 않은 아바타 변경사항을 버리고 아티스트를 변경할까요?")) return;
  ...
  const uploadFiles = async (selected: File[]) => {
    ...
    try {
      ...
      for (const file of selected) {
        const asset = await uploadAdminAsset("artist-assets", `${artistId}/avatars/${crypto.randomUUID()}.webp`, file);
      ...
      onToast(`${additions.length}개의 정사각형 아바타를 임시 목록에 추가했습니다. 저장하면 반영됩니다.`);
    } catch (error) {
      onError(error instanceof Error ? error.message : "아바타를 업로드하지 못했습니다.");
    } finally {
      setUploading(false);
  ...
  const selectFiles = (files: FileList | File[]) => {
    ...
    const invalid = selected.find((file) => !ACCEPTED_TYPES.has(file.type) || file.size > MAX_IMAGE_BYTES);
    if (invalid) return onError(`${invalid.name}: JPG, PNG, WebP 파일만 10MB까지 업로드할 수 있습니다.`);
    if (!selected.length) return;
    onError("");
  ...
  const confirmCrop = async () => {
    ...
    const cropped = [...cropQueue.cropped, await cropSquareImage(cropImageRef.current, cropQueue.files[cropQueue.index].name, cropZoom, cropX, cropY)];
  ...
  useEffect(() => {
    ...
    image.onload = () => { cropImageRef.current = image; setCropReady(true); };
    image.onerror = () => { onError("이미지를 읽을 수 없습니다."); closeCrop(); };
    image.src = url;
  ...
  useEffect(() => {
    ...
    if (!context) return;
    const crop = getSquareCrop(image.naturalWidth, image.naturalHeight, cropZoom, cropX, cropY);
  ...
  const moveItem = (index: number, direction: -1 | 1) => {
    ...
    setItems((current) => {
      ...
      return next.map((item, position) => ({ ...item, sort_order: position + 1 }));
    });
  };

  const removeItem = async () => {
    if (!deleteItem) return;
    const queued = uploadedAssets.current.find((asset) => asset.path === deleteItem.image_path);
    if (queued) {
      await discardDraftImageAssets(supabase, [queued]);
      uploadedAssets.current = uploadedAssets.current.filter((asset) => asset.path !== queued.path);
    }
    setItems((current) => current.filter((item) => item.id !== deleteItem.id).map((item, index) => ({ ...item, sort_order: index + 1 })));
    setDeleteItem(null);
  };
  ...
  const commitAssets = useCallback(async () => {
    ...
    onError("");
    const removed = snapshot.filter((item) => !items.some((current) => current.id === item.id));
    ...
    await finalizeDraftImageAssets(
      supabase,
      uploadedAssets.current,
      uploadedAssets.current.filter((asset) => currentPaths.has(asset.path)).map((asset) => asset.url),
      [],
    );
    uploadedAssets.current = [];
  ...
  }, [artistId, dirty, discardBackup, items, loadAssets, onError, onToast, snapshot]);

  useEffect(() => {
    if (!dirty || !artistId) return;
    const removedCount = snapshot.filter((item) => !items.some((current) => current.id === item.id)).length;
    const addedCount = items.filter((item) => !snapshot.some((current) => current.id === item.id)).length;
  ...
  const selectedArtist = useMemo(() => artists.find((artist) => artist.id === artistId), [artistId, artists]);

  return <section hidden={!active} className="avatar-manager" aria-labelledby="avatar-manager-title">
    {recovery && <div className="content-draft-recovery" role="status"><p><b>저장하지 않은 아바타 작업이 있습니다.</b></p><button type="button" onClick={discardBackup}>삭제</button><button type="button" onClick={restoreBackup}>복구</button></div>}
    <div className="content-section-heading settings-section-heading">
      <div><h3 id="avatar-manager-title">사용자 아바타</h3><p>아티스트별 선택 이미지를 등록하고 노출 순서를 관리합니다. 변경사항은 저장 전까지 공개되지 않습니다.</p></div>
      <ImagePlus aria-hidden="true" />
    </div>

    <div className="avatar-manager-toolbar">
      <div><span>아티스트</span><CustomSelect ariaLabel="아바타 아티스트 선택" value={artistId} onChange={(value) => void selectArtist(value)} options={artists.map((artist) => ({ value: artist.id, label: artist.eng_name || artist.name }))} /></div>
      <span>{selectedArtist ? `${selectedArtist.eng_name || selectedArtist.name} · ${items.length}개` : "등록된 아티스트 없음"}</span>
      <span className="avatar-manager-save-hint">상단 저장으로 일괄 반영</span>
    </div>
    ...
    {loading ? <AdminSkeleton variant="media" className="min-h-[250px]" /> : !items.length ? <div className="gallery-empty"><ImagePlus aria-hidden="true" /><b>등록된 사용자 아바타가 없습니다.</b><span>이미지를 추가한 뒤 변경사항을 저장해 주세요.</span></div> : <div className="avatar-manager-grid">
      {items.map((item, index) => <article key={item.id} className={`avatar-manager-card ${item.is_active ? "" : "is-inactive"}`}>
    ...
    {deleteItem && <DeleteConfirmDialog title="사용자 아바타를 삭제할까요?" description="저장하면 이 이미지는 선택 목록에서 제거됩니다. 현재 선택 중인 계정은 기본 아바타로 전환됩니다." confirmValue={`${selectedArtist?.eng_name || selectedArtist?.name || "아바타"} ${items.findIndex((item) => item.id === deleteItem.id) + 1}`} valueLabel="아바타" busy={false} onCancel={() => setDeleteItem(null)} onConfirm={() => void removeItem()} />}
    {cropQueue && <div className="avatar-crop-modal" role="dialog" aria-modal="true" aria-labelledby="avatar-crop-title">
      <div className="avatar-crop-card">
        <header><span><Crop aria-hidden="true" /></span><div><h3 id="avatar-crop-title">정사각형으로 자르기</h3><p>{cropQueue.files.length > 1 ? `${cropQueue.index + 1} / ${cropQueue.files.length} · ` : ""}확대와 위치를 조절해 사용할 영역을 맞춰 주세요.</p></div></header>
        <canvas ref={cropCanvasRef} className="avatar-crop-canvas" width="720" height="720" aria-label="아바타 자르기 미리보기" />
        <div className="avatar-crop-controls">
          <label><span>확대</span><input type="range" min="1" max="3" step="0.01" value={cropZoom} onChange={(event) => setCropZoom(Number(event.target.value))} /></label>
