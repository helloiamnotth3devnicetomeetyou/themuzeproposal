...
const primaryLinks = [
  {
    title: "메인 비주얼",
    description: "홈 화면에 노출할 앨범을 정리합니다.",
    href: "/admin/hero",
    icon: Image,
  },
  {
    title: "전체 공지",
    description: "사이트 전체 공지를 작성하고 공개합니다.",
    href: "/admin/notices",
    icon: Megaphone,
  },
];
  ...
  return (
    <main className={styles.page}>
      <header className={styles.heading}>
        <span>콘텐츠</span>
        <h1>공개할 이야기를 관리하세요</h1>
        <p>메인 화면과 공지, 아티스트별 콘텐츠를 빠르게 엽니다.</p>
      </header>
      {error && (
      ...
      <section
        ...
        aria-labelledby="artist-content-title"
      >
        <header>
          <div>
            <h2 id="artist-content-title">아티스트 콘텐츠</h2>
            <p>프로필, 멤버, 음반, 일정과 공지를 편집합니다.</p>
          </div>
          <Link
        ...
        {artists.length ? (
          <div className={styles.artistList}>
            {artists.map((artist) => (
              <Link
                key={artist.id}
                href={`/admin/artists/${artist.id}/profile`}
          ...
          <div className={styles.empty}>
            <UserRound aria-hidden="true" />
            <b>등록된 아티스트가 없습니다.</b>
            <Link href="/admin/artists/new/profile">첫 아티스트 추가하기</Link>
          </div>
        )}
      </section>
    </main>
