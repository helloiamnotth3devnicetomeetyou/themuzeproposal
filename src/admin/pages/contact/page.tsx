      ...
      !(await confirm({
        title:
          status === "answered"
            ? "답변 완료로 기록할까요?"
            : "문의를 종결할까요?",
        description: "처리 상태와 담당자 기록이 즉시 반영됩니다.",
        confirmLabel: "상태 변경",
      }))
    )
      return;
    ...
    return (
      <div className={`${base.page} ${base.detailPage} ${styles.fullPage}`}>
        <AdminToast
          ...
          onAction={undo ? () => void undoStatus() : undefined}
        />
        <button
          type="button"
            ...
            <section data-tour-id="contact-memo">
              ...
              <div className={base.noteActions}>
                ...
                <button
                  ...
                  onClick={() =>
                    void updateInquiry({ admin_note: note.trim() || null })
                  }
                >
                  메모 저장
                </button>
            ...
            <div>
              {statuses.map((status) => (
                <button
                  ...
                  onClick={() => void changeStatus(status.value)}
                >
                  {status.label}
                </button>
              ))}
          ...
          <div className={base.filters} data-tour-id="contact-filters">
            ...
            <span data-tour-id="contact-status-filter">
              <CustomSelect
                ...
                value={filter}
                onChange={(value) => {
                  setFilter(value as ContactStatus | "all");
                  setPage(1);
                }}
