...
type ContactDetailProps = {
  ...
  attachmentStatus: "idle" | "loading" | "ready" | "error";
    ...
    <div className={`${base.page} ${base.detailPage} ${styles.fullPage}`}>
      ...
      <article className={`${base.detailCard} ${styles.fullWidth}`} data-tour-id="contact-workspace">
        ...
        <div className={base.detailBody}>
          <div className={base.readingPane}>
            <section>
              <div className={base.sectionHeading}><h2>{isBusiness ? "제안 내용" : "문의 내용"}</h2></div>
              <p className={base.reportContent}>{viewing.message}</p>
            </section>

            <section>
              <div className={base.sectionHeading}><h2>문의자 정보</h2></div>
            ...
            {isBusiness && (
              <section>
                <div className={base.sectionHeading}><h2>제안서 첨부</h2></div>
                {viewing.attachment_path && attachmentStatus === "error" ? (
                  <div className={base.fileError} role="status">
                    <span>보안 링크를 만들지 못했습니다.</span>
                    <button type="button" onClick={onRetryAttachment}>다시 시도</button>
                  </div>
          ...
          <aside className={base.caseRail} aria-label="문의 처리 도구">
            <section>
              <div className={base.sectionHeading}><h2>우선순위 분류</h2></div>
              <dl className={base.infoGrid}>
                <div><dt>우선순위</dt><dd><span className={`${base.valueBadge} ${priority === "긴급" ? base.valueBadgeDanger : priority === "미분류" ? base.valueBadgePending : ""}`}>{priority}</span></dd></div>
                <div><dt>스팸 가능성</dt><dd><span className={`${base.valueBadge} ${viewing.is_likely_spam === null ? base.valueBadgePending : viewing.is_likely_spam ? base.valueBadgeDanger : base.valueBadgeSuccess}`}>{viewing.is_likely_spam === null ? "미분류" : viewing.is_likely_spam ? "의심" : "정상"}</span></dd></div>
              </dl>
              {viewing.ai_reasoning && <p className={base.reportContent}>{viewing.ai_reasoning}</p>}
            </section>

            <section>
              <div className={base.sectionHeading}><h2>열람 상태</h2></div>
            ...
            <section data-tour-id="contact-memo">
              <div className={base.sectionHeading}><h2>관리자 메모</h2></div>
