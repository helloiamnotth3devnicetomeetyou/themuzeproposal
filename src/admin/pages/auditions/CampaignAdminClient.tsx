...
export function CampaignBuilderAdmin({ campaignId }: { campaignId: string }) {
  ...
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><Link href="/admin/auditions/campaigns" className="audition-back"><ArrowLeft aria-hidden="true" /> 캠페인 목록</Link><h1>{campaign.title}</h1><p>질문을 끌어 순서를 바꾸고 저장하세요.</p></div><nav><Link className="admin-btn" href={`/admin/auditions/campaigns/${campaignId}/submissions`}><span data-tour-id="audition-status-prerequisite">지원서 심사</span></Link><button data-tour-id="audition-save" className="admin-btn admin-btn-primary" type="button" disabled={saving} onClick={() => void save()}><Save aria-hidden="true" /> {saving ? "저장 중…" : "저장"}</button></nav></header>
    ...
    <div className="audition-builder-layout"><section className="audition-builder-editor"><AdminLanguageTabs activeLang={language} onChange={setLanguage} values={{ ko: campaign.description_i18n?.ko ?? campaign.description, en: campaign.description_i18n?.en, ja: campaign.description_i18n?.ja }} />
      ...
      <div className="audition-question-workbench"><aside className="audition-question-list"><header><b>질문 목록</b><span>{fields.length}</span></header>{fields.map((field, index) => <div key={field.id} data-tour-id="audition-question-sort" className={`audition-question-item ${selectedField?.id === field.id ? "is-active" : ""}`} draggable onDragStart={() => setDragging(field.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => moveTo(field.id)}><GripVertical aria-hidden="true" /><button type="button" onClick={() => setSelectedFieldId(field.id)}><span>{index + 1}</span><b>{field.label_i18n.ko || field.label_i18n.en || field.label_i18n.ja || field.field_key}</b></button><button type="button" data-tour-id="audition-question-delete" onClick={() => remove(field.id)} aria-label="질문 삭제"><Trash2 aria-hidden="true" /></button></div>)}<button className="audition-add-field-btn" data-tour-id="audition-question-add" type="button" onClick={() => { const field = blankField(campaignId, fields.length); setFields((current) => [...current, field]); setSelectedFieldId(field.id); }}><Plus aria-hidden="true" /> <span data-tour-id="audition-builder-prerequisite">질문 추가</span></button></aside>
        <div className="audition-question-form">{selectedField ? <><header><div><h2>{selectedField.label_i18n.ko || "질문 설정"}</h2><p>언어 탭을 바꿔 각 언어의 질문을 입력하세요.</p></div></header><div className="audition-question-form-grid">
          ...
          {selectedField.field_type === "file" && <><label>파일당 최대 용량<input className="admin-input" type="number" min="1" max="30" value={selectedField.max_file_size_mb ?? 20} onChange={(event) => patchField(selectedField.id, { max_file_size_mb: Math.min(30, Number(event.target.value) || 20) })} /><span>1~30MB 사이로 설정할 수 있습니다.</span></label><div className="audition-question-wide"><span className="audition-control-label">허용 파일 종류</span><div className="audition-file-preset-chips">{FILE_PRESETS.map((preset) => { const checked = preset.types.every((type) => !selectedField.accepted_file_types.length || selectedField.accepted_file_types.includes(type)); return <label className={`audition-file-preset-chip ${checked ? "is-active" : ""}`} key={preset.label}><input type="checkbox" checked={checked} onChange={(event) => toggleFilePreset(preset.types, event.target.checked)} /><b>{preset.label}</b><span>{preset.hint}</span></label>; })}</div></div></>}
    ...
    </section><aside className="audition-builder-preview"><div className="audition-preview-toolbar"><b>지원서 미리보기</b></div><div className="audition-preview-paper"><header><h2>{campaign.title}</h2><p>{campaignDescription(campaign, language)}</p></header><div className="audition-preview-fields">{fields.map((field) => <div className="audition-preview-select" key={field.id}><FieldPreview field={field} locale={language} /></div>)}</div><button type="button" disabled>제출 내용 검토</button></div></aside></div>
  </div>;
}
...
export function SubmissionReviewAdmin({ campaignId }: { campaignId: string }) {
  ...
  const filtered = useMemo(() => submissions.filter((item) => (
    ...
    && (!query.trim() || JSON.stringify(item.answers).toLowerCase().includes(query.trim().toLowerCase()))
  ...
  const open = async (submission: AuditionSubmission) => { setSelected(submission); setNote(submission.reviewer_notes || ""); const files = Object.values(submission.answers).filter((answer): answer is { path: string; name: string; size: number; mimeType: string } => typeof answer === "object" && !Array.isArray(answer) && "path" in answer); const pairs = await Promise.all(files.map(async (file) => [file.path, await fetchSignedFileUrl("audition-attachments", file.path, file.name)] as const)); setSigned(Object.fromEntries(pairs)); };
  const applyStatus = async (status: string) => { if (!selected) return false; setSavingReview(true); const { data, error } = await supabase.rpc("review_audition_submission", { p_submission_id: selected.id, p_status: status, p_reviewer_notes: note.trim() || null }); setSavingReview(false); const patch = Array.isArray(data) ? data[0] : data; if (error || !patch) { setToast("저장하지 못했습니다."); return false; } const next = { ...selected, ...patch } as AuditionSubmission; setSelected(next); setSubmissions((current) => current.map((item) => item.id === next.id ? next : item)); window.dispatchEvent(new Event("admin-inbox-changed")); return true; };
  const update = async (status: string) => { if (!selected || status === selected.status) return; if (["accepted", "rejected"].includes(status) && !await confirm({ title: status === "accepted" ? "합격으로 변경할까요?" : "불합격으로 변경할까요?", description: "심사 결과와 담당자가 즉시 기록됩니다.", confirmLabel: "심사 결과 변경" })) return; const previousStatus = selected.status; if (await applyStatus(status)) { setUndoStatus(previousStatus); setToast("심사 상태를 변경했습니다."); } };
  const saveNote = async () => { if (selected && await applyStatus(selected.status)) setToast("심사 메모를 저장했습니다."); };
  const undo = async () => { if (!undoStatus) return; if (await applyStatus(undoStatus)) { setUndoStatus(null); setToast("이전 심사 상태로 되돌렸습니다."); } };
  if (selected) {
    ...
    return <div className="audition-campaign-page"><AdminToast message={toast} actionLabel={undoStatus ? "되돌리기" : undefined} onAction={undoStatus ? () => void undo() : undefined} /><button className="audition-back" type="button" onClick={() => setSelected(null)}><ArrowLeft aria-hidden="true" /> 지원서 목록</button><section className="audition-review-detail"><header><div className="audition-review-identity"><span className="audition-review-avatar">{avatarUrl ? <AdminAssetImage src={avatarUrl} alt={`${applicantName} 아바타`} sizes="56px" /> : <b aria-hidden="true">{(applicantEmail[0] || applicantName[0] || "A").toUpperCase()}</b>}</span><div><small>{campaign?.title}</small><h1>{applicantName}</h1><p>{applicantEmail}</p></div></div><span>{REVIEW_STATUSES.find((item) => item.value === selected.status)?.label}</span></header><dl>{selected.form_snapshot.map((field) => { const answer = selected.answers[field.field_key]; const file = typeof answer === "object" && !Array.isArray(answer) && "path" in answer ? answer : null; return <div key={field.id}><dt>{fieldLabel(field, "ko")}</dt><dd>{file ? <a href={signed[file.path] || undefined} target="_blank" rel="noreferrer">{file.name} <ExternalLink aria-hidden="true" /></a> : Array.isArray(answer) ? answer.join(", ") : String(answer || "-")}</dd></div>; })}</dl><label>심사 메모<textarea className="admin-input" value={note} onChange={(event) => setNote(event.target.value)} /></label><button className="admin-btn admin-btn-secondary" type="button" disabled={savingReview || note.trim() === (selected.reviewer_notes || "")} onClick={() => void saveNote()}><Save aria-hidden="true" /> {savingReview ? "저장 중" : "메모 저장"}</button><footer data-tour-id="audition-status">{REVIEW_STATUSES.map((status) => <button type="button" disabled={savingReview} className={selected.status === status.value ? "is-active" : ""} key={status.value} onClick={() => void update(status.value)}>{status.label}</button>)}</footer></section></div>;
  }
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><Link href="/admin/auditions/campaigns" className="audition-back"><ArrowLeft aria-hidden="true" /> 캠페인 목록</Link><p className="audition-review-breadcrumb">캠페인 목록 · {campaign?.title} · 지원서 심사</p><h1>{campaign?.title || "지원서 심사"}</h1><p>{filtered.length} / {submissions.length}개의 지원서</p></div><div className="audition-review-tools"><input className="admin-input" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="답변 검색" /><select className="admin-input" aria-label="심사 상태 필터" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">모든 상태</option>{REVIEW_STATUSES.map((status) => <option key={status.value} value={status.value}>{status.label}</option>)}</select></div></header><div className="audition-review-list">{filtered.map((submission) => { const primary = submission.form_snapshot.find((field) => field.is_primary_label); return <button type="button" data-tour-id="audition-status-prerequisite" key={submission.id} onClick={() => void open(submission)}><span>{new Date(submission.created_at).toLocaleDateString("ko-KR")}</span><b>{primary ? String(submission.answers[primary.field_key] || "이름 없음") : submission.id.slice(0, 8)}</b><em>{REVIEW_STATUSES.find((item) => item.value === submission.status)?.label}</em></button>; })}{!filtered.length && <p className="audition-review-empty">조건에 맞는 지원서가 없습니다.</p>}</div></div>;
}
