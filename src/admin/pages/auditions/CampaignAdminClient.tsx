...
export function SubmissionReviewAdmin({ campaignId }: { campaignId: string }) {
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([supabase.from("audition_campaigns").select("*").eq("id", campaignId).single(), supabase.rpc("get_admin_audition_submissions", { p_campaign_id: campaignId })]).then(([c, s]) => {
      if (!active) return;
