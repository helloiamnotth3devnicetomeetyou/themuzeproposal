...
export function CampaignBuilderAdmin({ campaignId }: { campaignId: string }) {
  ...
  if (!campaign) return message ? <div className="audition-campaign-page">{message}</div> : <AdminSkeleton variant="workbench" className="min-h-[420px]" />;
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><Link href="/admin/auditions/campaigns" className="audition-back"><ArrowLeft aria-hidden="true" /> 캠페인 목록</Link><h1>{campaign.title}</h1><p>질문을 끌어 순서를 바꾸고 저장하세요.</p></div><nav><Link className="admin-btn" href={`/admin/auditions/campaigns/${campaignId}/submissions`}>지원서 심사</Link><button data-tour-id="audition-save" className="admin-btn admin-btn-primary" type="button" disabled={saving} onClick={() => void save()}><Save aria-hidden="true" /> {saving ? "저장 중…" : "저장"}</button></nav></header>
