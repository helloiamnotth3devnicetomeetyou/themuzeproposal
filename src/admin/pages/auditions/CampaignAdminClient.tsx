...
export function CampaignListAdmin() {
  ...
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><h1>캠페인 관리</h1><p>캠페인별 지원서와 질문을 관리합니다.</p></div><button className="admin-btn admin-btn-primary" type="button" onClick={() => void create()}><Plus aria-hidden="true" /> 새 캠페인</button></header>
    {error && <div className="hero-admin-alert is-error" role="alert">{error}</div>}
    {loading ? <p>불러오는 중…</p> : <div className="audition-campaign-list">{campaigns.map((campaign) => <article key={campaign.id}><div><span className={`audition-session-badge ${campaign.is_active ? "is-open" : "is-closed"}`}>{campaign.is_active ? "ACTIVE" : "DRAFT"}</span><h2>{campaign.title}</h2><p>{campaign.description || "소개 없음"}</p></div><nav><button type="button" onClick={() => void toggle(campaign)}>{campaign.is_active ? "비활성화" : "활성화"}</button><Link href={`/admin/auditions/campaigns/${campaign.id}/submissions`}><Inbox aria-hidden="true" /> 심사</Link><Link href={`/admin/auditions/campaigns/${campaign.id}/builder`}>폼 편집</Link></nav></article>)}</div>}
  </div>;
}
...
export function CampaignBuilderAdmin({ campaignId }: { campaignId: string }) {
  ...
  if (!campaign) return <div className="audition-campaign-page">{message || "불러오는 중…"}</div>;
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><Link href="/admin/auditions/campaigns" className="audition-back"><ArrowLeft aria-hidden="true" /> 캠페인 목록</Link><h1>{campaign.title}</h1><p>질문을 끌어 순서를 바꾸고 저장하세요.</p></div><nav><Link className="admin-btn" href={`/admin/auditions/campaigns/${campaignId}/submissions`}>지원서 심사</Link><button className="admin-btn admin-btn-primary" type="button" disabled={saving} onClick={() => void save()}><Save aria-hidden="true" /> {saving ? "저장 중…" : "저장"}</button></nav></header>
    ...
    <div className="audition-builder-layout"><section className="audition-builder-editor">
      ...
      <div className="audition-question-workbench"><aside className="audition-question-list"><header><b>질문 목록</b><span>{fields.length}</span></header>{fields.map((field, index) => <div key={field.id} className={`audition-question-item ${selectedField?.id === field.id ? "is-active" : ""}`} draggable onDragStart={() => setDragging(field.id)} onDragOver={(event) => event.preventDefault()} onDrop={() => moveTo(field.id)}><GripVertical aria-hidden="true" /><button type="button" onClick={() => setSelectedFieldId(field.id)}><span>{index + 1}</span><b>{field.label_i18n.ko || field.label_i18n.en || field.label_i18n.ja || field.field_key}</b></button><button type="button" onClick={() => remove(field.id)} aria-label="질문 삭제"><Trash2 aria-hidden="true" /></button></div>)}<button className="audition-add-field-btn" type="button" onClick={() => { const field = blankField(campaignId, fields.length); setFields((current) => [...current, field]); setSelectedFieldId(field.id); }}><Plus aria-hidden="true" /> 질문 추가</button></aside>
...
export function SubmissionReviewAdmin({ campaignId }: { campaignId: string }) {
  ...
  useEffect(() => {
    let active = true;
    void Promise.all([supabase.from("audition_campaigns").select("*").eq("id", campaignId).single(), supabase.from("audition_submissions").select("id,campaign_id,answers,form_snapshot,status,reviewer_notes,reviewed_by,reviewed_at,created_at").eq("campaign_id", campaignId).order("created_at", { ascending: false })]).then(([c, s]) => {
      if (!active) return;
      if (c.data) setCampaign(c.data as AuditionCampaign);
      if (s.data) setSubmissions(s.data as AuditionSubmission[]);
    });
    return () => { active = false; };
  }, [campaignId]);
  ...
  const open = async (submission: AuditionSubmission) => { setSelected(submission); setNote(submission.reviewer_notes || ""); const files = Object.values(submission.answers).filter((answer): answer is { path: string; name: string; size: number; mimeType: string } => typeof answer === "object" && !Array.isArray(answer) && "path" in answer); const pairs = await Promise.all(files.map(async (file) => [file.path, (await supabase.storage.from("audition-attachments").createSignedUrl(file.path, 900)).data?.signedUrl || ""] as const)); setSigned(Object.fromEntries(pairs)); };
  const update = async (status: string) => { if (!selected) return; const { data: { user } } = await supabase.auth.getUser(); const patch = { status, reviewer_notes: note.trim() || null, reviewed_by: user?.id ?? null, reviewed_at: new Date().toISOString() }; const { error } = await supabase.from("audition_submissions").update(patch).eq("id", selected.id); if (!error) { const next = { ...selected, ...patch } as AuditionSubmission; setSelected(next); setSubmissions((current) => current.map((item) => item.id === next.id ? next : item)); } };
  if (selected) return <div className="audition-campaign-page"><button className="audition-back" type="button" onClick={() => setSelected(null)}><ArrowLeft aria-hidden="true" /> 지원서 목록</button><section className="audition-review-detail"><header><h1>{campaign?.title}</h1><span>{REVIEW_STATUSES.find((item) => item.value === selected.status)?.label}</span></header><dl>{selected.form_snapshot.map((field) => { const answer = selected.answers[field.field_key]; const file = typeof answer === "object" && !Array.isArray(answer) && "path" in answer ? answer : null; return <div key={field.id}><dt>{fieldLabel(field, "ko")}</dt><dd>{file ? <a href={signed[file.path] || undefined} target="_blank" rel="noreferrer">{file.name} <ExternalLink aria-hidden="true" /></a> : Array.isArray(answer) ? answer.join(", ") : String(answer || "-")}</dd></div>; })}</dl><label>심사 메모<textarea className="admin-input" value={note} onChange={(event) => setNote(event.target.value)} /></label><footer>{REVIEW_STATUSES.map((status) => <button type="button" className={selected.status === status.value ? "is-active" : ""} key={status.value} onClick={() => void update(status.value)}>{status.label}</button>)}</footer></section></div>;
  return <div className="audition-campaign-page"><header className="audition-campaign-heading"><div><Link href={`/admin/auditions/campaigns/${campaignId}/builder`} className="audition-back"><ArrowLeft aria-hidden="true" /> 폼 빌더</Link><h1>{campaign?.title || "지원서 심사"}</h1><p>{submissions.length}개의 지원서</p></div><input className="admin-input" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="답변 검색" /></header><div className="audition-review-list">{filtered.map((submission) => { const primary = submission.form_snapshot.find((field) => field.is_primary_label); return <button type="button" key={submission.id} onClick={() => void open(submission)}><span>{new Date(submission.created_at).toLocaleDateString("ko-KR")}</span><b>{primary ? String(submission.answers[primary.field_key] || "이름 없음") : submission.id.slice(0, 8)}</b><em>{REVIEW_STATUSES.find((item) => item.value === submission.status)?.label}</em></button>; })}</div></div>;
}
