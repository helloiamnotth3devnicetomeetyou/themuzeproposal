...
export function SubmissionReviewAdmin({ campaignId }: { campaignId: string }) {
  ...
  const applyStatus = async (status: string) => { if (!selected) return; const { data, error } = await supabase.rpc("review_audition_submission", { p_submission_id: selected.id, p_status: status, p_reviewer_notes: note.trim() || null }); const patch = Array.isArray(data) ? data[0] : data; if (!error && patch) { const next = { ...selected, ...patch } as AuditionSubmission; setSelected(next); setSubmissions((current) => current.map((item) => item.id === next.id ? next : item)); window.dispatchEvent(new Event("admin-inbox-changed")); } };
