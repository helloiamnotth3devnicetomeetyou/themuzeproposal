...
vi.mock("@/core/supabase/service", () => ({ createServiceRoleClient: vi.fn() }));
...
describe("/api/admin/accounts role safety", () => {
  beforeEach(() => {
    ...
    mocks.createSessionClient.mockResolvedValue({ auth: { getUser: mocks.getUser }, rpc: mocks.rpc });
  });
  ...
  it("delegates role invariants to the transactional database function", async () => {
    mocks.rpc.mockResolvedValueOnce({ error: { message: "CANNOT_CHANGE_OWN_ROLE" } });
  ...
  it("changes another administrator through one RPC", async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledWith("set_admin_role", { p_target_id: targetId, p_role: "super_admin" });
  });
});
