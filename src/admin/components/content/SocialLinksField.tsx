...
export default function SocialLinksField({
  ...
  const patchLink = (id: string, patch: Partial<SocialLink>) =>
    onChange(
      value.map((link) => (link.id === id ? { ...link, ...patch } : link)),
    );
  ...
  return (
    <div className="social-link-editor">
      ...
      {!value.length && (
        <button
          type="button"
          className="social-link-empty"
          onClick={() => onChange([createLink()])}
        >
          <Plus aria-hidden="true" />
      ...
      <div className="social-link-list">
        {value.map((link, index) => {
          const platform =
            detectSocialPlatform(link.url) !== "other"
              ? detectSocialPlatform(link.url)
              : link.platform;
          const invalid = Boolean(link.url) && !isValidLink(link.url);
          return (
            <div
              className={`social-link-row ${platform === "other" ? "has-custom-label" : ""}`}
              key={link.id}
            >
              <span className="social-link-index">
              ...
              <div className="music-field social-link-platform">
                ...
                <CustomSelect
                  ...
                  onChange={(nextPlatform) =>
                    patchLink(link.id, { platform: nextPlatform })
                  }
                  options={PLATFORM_OPTIONS.map(([optionValue, label]) => ({
                    value: optionValue,
                    label,
                  }))}
                />
              ...
              {platform === "other" && (
                <label className="music-field">
                  ...
                  <input
                    ...
                    placeholder="플랫폼과 계정 이름"
                  />
                </label>
              )}
              <label
                className={`music-field social-link-address ${invalid ? "is-invalid" : ""}`}
              >
                <span>
                  계정 링크 <b>*</b>
                </span>
                <input
                  ...
                  onChange={(event) => {
                    ...
                    patchLink(link.id, {
                      url,
                      ...(detected !== "other" ? { platform: detected } : {}),
                    });
                  }}
                  placeholder="https://..."
                />
                {invalid && (
                  <small>
                    http:// 또는 https://로 시작하는 링크를 입력하세요.
                  </small>
                )}
              </label>
              <button
                ...
                aria-label={`${SOCIAL_LABELS[platform] || "소셜"} 계정 삭제`}
                onClick={() =>
                  onChange(value.filter((item) => item.id !== link.id))
                }
              >
