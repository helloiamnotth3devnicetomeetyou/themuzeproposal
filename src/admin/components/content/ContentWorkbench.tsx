    ...
    <div className={`content-workbench ${className}${railOpen ? " is-rail-open" : ""}`.trim()}>
      ...
      <section className="content-workbench-stage">
        ...
        <header className="content-workbench-header">
          ...
          {railLabel && <button ref={railTriggerRef} type="button" className="content-mobile-rail-trigger" aria-expanded={railOpen} aria-controls="admin-mobile-rail" onClick={() => setRailOpen((open) => !open)}><List aria-hidden="true" />{railLabel}</button>}
