...
async function getSuggestedColor(file: File) {
  ...
  try {
    ...
    return count
      ? `#${hex(r / count)}${hex(g / count)}${hex(b / count)}`.toUpperCase()
      : BRAND_PINK_HEX;
  } finally {
...
export function CoverAssetField({
  artistId,
  albumId,
  value,
  onUploaded,
  ...
  const choose = async (file?: File) => {
    ...
    if (!IMAGE_TYPES.includes(file.type))
      return onError("커버는 JPG, PNG, WebP 파일만 올릴 수 있습니다.");
    if (file.size > 30 * 1024 * 1024)
      return onError("커버 파일은 30MB 이하여야 합니다.");
    setBusy(true);
    try {
      ...
      const [asset, suggested] = await Promise.all([
        uploadAdminAsset(
          "album-covers",
          `${artistId}/${albumId}/cover/${crypto.randomUUID()}.webp`,
          converted,
        ),
      ...
      onError(
        cause instanceof Error ? cause.message : "커버 업로드에 실패했습니다.",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <div
      className={`music-cover-field music-image-dropzone ${dragging ? "is-dragging" : ""}`}
      onDragEnter={(event) => {
        event.preventDefault();
    ...
    >
      <div className="music-cover-preview">
        {value ? (
          <AdminAssetImage src={value} alt="앨범 커버" sizes="240px" />
        ) : (
          <span>커버 없음</span>
        )}
      </div>
      <div className="music-asset-copy">
        <b>앨범 커버</b>
        <p>
          파일을 여기에 놓거나 선택하세요 · 원본 그대로 저장 · JPG, PNG, WebP ·
          최대 30MB
        </p>
        <label className="music-upload-button" htmlFor={inputId}>
          {busy ? "업로드 중…" : value ? "이미지 교체" : "파일 선택"}
        </label>
        <input
          id={inputId}
          className="sr-only"
...
export function HeroAssetField({
  artistId,
  albumId,
  value,
  onUploaded,
  ...
  const choose = async (file?: File) => {
    if (!file) return;
    if (!IMAGE_TYPES.includes(file.type))
      return onError("히어로 이미지는 JPG, PNG, WebP 파일만 올릴 수 있습니다.");
    if (file.size > 30 * 1024 * 1024)
      return onError("히어로 이미지는 30MB 이하여야 합니다.");
    setBusy(true);
    try {
      const converted = await toWebP(file);
  ...
  return (
    <div
      className={`music-hero-field music-image-dropzone ${dragging ? "is-dragging" : ""}`}
      onDragEnter={(event) => {
        event.preventDefault();
        if (!busy) setDragging(true);
      }}
      ...
      onDrop={drop}
    >
      <div className="music-hero-preview">
        {value ? (
          <AdminAssetImage
            src={value}
            alt="메인 슬라이드 히어로"
          ...
          <span>16:9 HERO IMAGE</span>
        )}
      </div>
      <div className="music-asset-copy">
        <b>메인 슬라이드 히어로 이미지</b>
        <p>
          파일을 여기에 놓거나 선택하세요 · 원본 그대로 저장 · 16:9 권장 · JPG,
          PNG, WebP · 최대 30MB
        </p>
        <div className="music-hero-actions">
          ...
          {value && (
            <button
              type="button"
              className="music-upload-button is-quiet"
              disabled={busy}
              onClick={onClear}
            >
              제거
...
export function TrackAssetField({
  label,
  hint,
  accept,
  maxBytes,
  ...
  const choose = async (file?: File) => {
    ...
    const acceptedByExtension = accepted.some(
      (item) => item.startsWith(".") && file.name.toLowerCase().endsWith(item),
    );
    if (!accepted.includes(file.type) && !acceptedByExtension)
      return onError(`${label} 파일 형식을 확인해 주세요.`);
    if (file.size > maxBytes)
      return onError(`${label} 파일 용량이 제한을 넘었습니다.`);
    setBusy(true);
    try {
      const isSvg =
        kind === "logo" &&
  ...
  return (
    ...
    >
      {kind === "logo" && value ? (
        <span className="track-asset-logo-preview">
          <AdminAssetImage
            src={value}
            alt="업로드한 타이포 로고"
            sizes="160px"
            className={
              /\.svg(?:$|\?)/i.test(value) ? "is-theme-svg" : undefined
            }
          />
        </span>
      ) : (
      ...
      {value && (
        <button type="button" onClick={onClear} aria-label={`${label} 제거`}>
          <X aria-hidden="true" />
        </button>
      )}
      <input
