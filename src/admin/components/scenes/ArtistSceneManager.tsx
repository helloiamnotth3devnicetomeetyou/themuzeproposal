...
export default function ArtistSceneManager({
  ...
  const uploadScenes = async (files: FileList) => {
    ...
    if (invalid)
      return onError(
        `${invalid.name}: JPG, PNG, WebP만 가능하며 파일은 최대 20MB입니다.`,
      );
    setBusy(true);
    onError("");
    try {
  ...
  const uploadMask = async (file: File) => {
    if (
      ...
      return onError("외곽선을 먼저 저장한 뒤 정밀 마스크를 올려주세요.");
    ...
    <div className={styles.manager} data-tour-id="artist-scenes">
      ...
      <div className={styles.toolbar}>
        ...
        <input
          ...
          hidden
          onChange={(event) =>
            event.target.files && void uploadScenes(event.target.files)
          }
        />
      </div>
          ...
          {selectedScene && (
            <div className={styles.sceneSettings} data-tour-id="scene-settings">
              <FormField
                ...
                onChangeKo={(value) =>
                  patchScene({ title: value, title_ko: value })
                }
                onChangeEn={(value) => patchScene({ title_en: value })}
                onChangeJa={(value) => patchScene({ title_ja: value })}
              />
              <label className={styles.sceneLinkField}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    patchScene({ link_url: event.target.value })
                  }
              ...
              <label className={styles.toggle}>
                <input
                  ...
                  onChange={(event) =>
                    patchScene({ is_published: event.target.checked })
                  }
                />
                <span>공개</span>
              </label>
              ...
              <aside className={styles.outlineTools}>
                ...
                <input
                  ...
                  onChange={(event) =>
                    ...
                    void uploadMask(event.target.files[0])
                  }
                />
