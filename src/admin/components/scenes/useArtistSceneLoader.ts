...
import { normalizeScene, sceneSelect, type MemberLookup } from "./artist-scene-editor-model";
import type { ArtistScene } from "@/core/utils/artist-scenes";

export function useArtistSceneLoader(
  artistId: string | null,
  onError: (message: string) => void,
  setScenes: (value: ArtistScene[] | ((current: ArtistScene[]) => ArtistScene[])) => void,
  setSnapshot: (value: ArtistScene[]) => void,
  setMembers: (value: MemberLookup[]) => void,
  setSelectedSceneId: (value: string | null | ((current: string | null) => string | null)) => void,
) {
  ...
  const load = useCallback(async (preferredSceneId?: string) => {
    ...
    setScenes(nextScenes); setSnapshot(nextScenes); setMembers((memberResult.data as MemberLookup[] | null) ?? []);
    setSelectedSceneId((current) => { const candidate = preferredSceneId || current; return candidate && nextScenes.some((scene) => scene.id === candidate) ? candidate : nextScenes[0]?.id ?? null; });
