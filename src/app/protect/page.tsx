...
export default function ProtectPage() {
  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      ...
      if (!active) return;
      setUserEmail(session.user.email || "로그인 계정");
      if (artistResult.error || reportResult.error) setError("신고 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
      setArtists((artistResult.data ?? []) as Artist[]);
      setMyReports((reportResult.data ?? []) as MyReport[]);
  ...
  const submitReport = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      for (const file of files) {
        ...
        const path = `${session.user.id}/${crypto.randomUUID()}.${extension.toLowerCase()}`;
  ...
  if (checkingSession) {
    return <LoadingIndicator label="로그인 상태를 확인하는 중" className="min-h-screen" />;
  }
