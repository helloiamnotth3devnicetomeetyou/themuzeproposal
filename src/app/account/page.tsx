...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    const loadAccount = async () => {
      try {
        ...
        if (!active) return;
        const accountName = profile?.name || user.user_metadata?.name || "";
        setName(accountName);
        setEmail(user.email || "");
        setOriginalEmail(user.email || "");
      } catch {
        if (active) setProfileStatus({ type: "error", message: t.loadError });
      } finally {
  ...
  if (loading) {
    return <div className={styles.loading}><LoadingIndicator label={t.loading} /></div>;
  }
