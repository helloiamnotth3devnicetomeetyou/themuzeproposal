...
const seoulDateKey = (date: Date) => new Date(date.getTime() + 9 * 60 * 60 * 1000).toISOString().slice(0, 10);
...
const readRows = async (response: Response) => {
  ...
  if (!Array.isArray(payload.data)) throw new Error("Invalid Vercel Web Analytics response");
  return payload.data;
};

const parseTrend = (rows: unknown[]) => rows.flatMap((row): VercelVisit[] => {
  if (!row || typeof row !== "object") return [];
  const { timestamp, pageviews, visitors } = row as Record<string, unknown>;
  if (typeof timestamp !== "string" || typeof pageviews !== "number" || typeof visitors !== "number") return [];
  return [{ timestamp, pageviews, visitors }];
});

const parseBreakdown = (rows: unknown[], dimension: string) => rows.flatMap((row): Breakdown[] => {
  if (!row || typeof row !== "object") return [];
  const record = row as Record<string, unknown>;
  if (typeof record[dimension] !== "string" || typeof record.pageviews !== "number" || typeof record.visitors !== "number") return [];
...
export async function GET(request: Request) {
  ...
  const baseParams = new URLSearchParams({ projectId, since: since.toISOString().slice(0, 10), until: todayKey });
  if (process.env.VERCEL_TEAM_ID) baseParams.set("teamId", process.env.VERCEL_TEAM_ID);

  const query = (by: string, limit = 100) => {
    const params = new URLSearchParams(baseParams);
    params.set("by", by);
    params.set("limit", String(limit));
    return fetch(`https://api.vercel.com/v1/query/web-analytics/visits/aggregate?${params}`, {
  ...
  try {
    ...
    const [pathRows = [], countryRows = [], deviceRows = [], osRows = [], browserRows = [], environmentRows = [], referrerRows = []] = breakdownRows;
    ...
    if (error instanceof VercelApiError && error.status === 402) {
      return Response.json({
        ...
        error: "현재 Vercel 요금제의 조회 가능 기간을 초과했습니다. Hobby 플랜은 최근 1개월까지만 조회할 수 있습니다.",
      });
    }
    console.error("Failed to load Vercel page statistics", error);
    return Response.json({ ...emptyStats(true, range), error: "페이지 통계를 불러오지 못했습니다." }, { status: 502 });
  }
}
