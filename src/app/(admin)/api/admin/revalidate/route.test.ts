...
describe("POST /api/admin/revalidate", () => {
  ...
  it("only revalidates the explicit tag allowlist", async () => {
    ...
    const response = await POST(
      request({ tags: ["public-home-slides", "public-artist-title"] }),
    );
    expect(response.status).toBe(200);
    expect(mocks.revalidateTag).toHaveBeenCalledWith("public-home-slides", {
      expire: 0,
    });
    expect(mocks.revalidateTag).toHaveBeenCalledWith("public-artist-title", {
      expire: 0,
    });
  });
});
