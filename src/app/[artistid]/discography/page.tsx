...
export default function Discography() {
  ...
  if (loading || !album) {
    ...
    return (
      <main className="min-h-screen flex items-center justify-center px-6" style={{ backgroundColor: "var(--palette-050505)" }}>
        {loading ? <LoadingIndicator label={message} className="text-[var(--palette-9ca3af)]" /> : <p className="text-sm text-[var(--palette-9ca3af)]">{message}</p>}
      </main>
    );
  }

  return (
    <main className="h-screen w-full relative overflow-hidden flex flex-col" style={{ backgroundColor: "var(--palette-050505)" }}>
      <audio
        ref={audioRef}
        preload="metadata"
          ...
          >
              ...
              return (
                ...
                >
                  <div
                    ...
                    style={{
                      border: isActiveTrack ? `3px solid ${album.color}` : "2px solid var(--alpha-0f0f0f-95)",
                  ...
                  >
                    ...
                    <div className="absolute inset-0 rounded-full pointer-events-none"
                      style={{ background: "radial-gradient(circle, transparent 26%, var(--alpha-000000-3) 28%, transparent 30%, transparent 46%, var(--alpha-000000-2) 48%, transparent 50%, transparent 66%, var(--alpha-000000-12) 68%, transparent 70%, transparent 86%, var(--alpha-000000-08) 88%, transparent 90%)" }}
                    />
            ...
            <div
              ...
              style={{
                ...
                border: `1px solid ${showDiscs ? `${album.color}40` : "var(--alpha-ffffff-05)"}`,
                willChange: "transform",
              }}
            >
              ...
              <div
                ...
                style={{ backgroundColor: album.color, color: "var(--color-static-black)", boxShadow: `0 4px 18px ${album.color}35` }}
              >
              ...
              <div className="absolute top-3 right-3 w-7 h-7 rounded-full bg-[var(--alpha-000000-4)] backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 border border-[var(--alpha-ffffff-08)]">
                <LuChevronRight className={`w-3.5 h-3.5 text-[var(--color-static-white)] transition-transform duration-500 ${showDiscs ? "rotate-180" : ""}`} aria-hidden="true" />
              </div>
            </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 w-full relative z-20 h-full max-h-[600px]">
          ...
          <div className="shrink-0">
            ...
            <div className="flex items-center justify-between mt-1">
              <h2 className="font-hero text-4xl font-black leading-none tracking-tight text-[var(--color-static-white)] md:text-5xl">{album.title}</h2>
              <div className="flex gap-2">
                <a href={album.links?.spotify || "#"} target="_blank" aria-label={`${album.title} on Spotify`} className="w-8 h-8 rounded-full border border-[var(--alpha-ffffff-1)] flex items-center justify-center hover:bg-[var(--alpha-ffffff-1)] transition-colors text-[var(--color-static-white)] hover:text-[var(--palette-e5e7eb)]">
                  <SiSpotify className="w-4 h-4" aria-hidden="true" />
                </a>
              </div>
            </div>
            <p className="text-[11px] text-[var(--palette-6b7280)] font-medium tracking-wider mt-2">{album.releaseDate}</p>
          </div>

          {/* Tabs Navigation */}
          <div className="flex gap-5 border-b border-[var(--alpha-ffffff-1)] pb-2 mt-1 relative shrink-0">
            {(["concept", "intro", "members"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`text-[11px] font-bold tracking-widest uppercase transition-colors duration-300 relative pb-1 ${activeTab === tab ? "text-[var(--color-static-white)]" : "text-[var(--palette-6b7280)] hover:text-[var(--palette-d1d5db)]"}`}
              >
                {tab === "concept" ? "TITLE IMAGE" : tab === "intro" ? "TRACK INTRO" : "MEMBERS"}
                {activeTab === tab && (
                  <span className="absolute -bottom-[9px] left-0 right-0 h-[2px] bg-[var(--color-static-white)] rounded-t-full shadow-[0_0_8px_var(--alpha-ffffff-6)]" />
                )}
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 relative min-h-[300px]">
            ...
            {activeTab === "intro" && (
              <div className="absolute inset-0 flex flex-col gap-4 animate-slideIn">
                <p className="text-sm text-[var(--palette-9ca3af)] font-light leading-relaxed max-w-md shrink-0">{album.desc[locale]}</p>

                {/* Player */}
                <div className="p-4 rounded-2xl flex flex-col gap-3 shrink-0"
                  style={{ backgroundColor: "var(--alpha-ffffff-025)", border: "1px solid var(--alpha-ffffff-05)", backdropFilter: "blur(12px)" }}
                >
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <span className="text-[8px] text-[var(--palette-4b5563)] font-black tracking-[0.15em] block">NOW PLAYING</span>
                      <span className="text-base font-bold text-[var(--color-static-white)] block truncate mt-0.5">{album.tracks[currentTrackIndex]?.title}</span>
                    </div>
                    <span className="text-[10px] text-[var(--palette-6b7280)] shrink-0">{time.current} / {time.total}</span>
                  </div>
                  ...
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center mt-1">
                    ...
                    <div className="flex items-center justify-center gap-6">
                      <button onClick={handlePrev} className="text-[var(--palette-6b7280)] hover:text-[var(--color-static-white)] transition-colors duration-200" aria-label="이전 트랙">
                        <LuChevronLeft className="w-5 h-5" aria-hidden="true" />
                      </button>
                      <button onClick={togglePlay} disabled={!album.tracks[currentTrackIndex]?.audioUrl} title={album.tracks[currentTrackIndex]?.audioUrl ? "재생" : "등록된 MP3가 없습니다"} className="w-10 h-10 rounded-full flex items-center justify-center hover:scale-110 active:scale-95 disabled:opacity-30 disabled:hover:scale-100" style={{ backgroundColor: album.color, color: "var(--color-static-black)", transition: "background-color 0.5s, transform 0.2s" }}>
                        {isPlaying ? (
                      ...
                      <button onClick={handleNext} className="text-[var(--palette-6b7280)] hover:text-[var(--color-static-white)] transition-colors duration-200" aria-label="다음 트랙">
                  ...
                  {album.tracks.map((track, idx) => {
                    ...
                    return (
                      <div key={track.title} className="flex items-center rounded-xl group/track relative shrink-0 pr-2"
                        style={{
                          backgroundColor: isActive ? "var(--alpha-ffffff-06)" : (isHovered ? "var(--alpha-ffffff-04)" : undefined),
                      ...
                      >
                        <button type="button" onClick={() => playTrack(idx)} className="flex flex-1 items-center gap-3 min-w-0 p-2.5 text-left cursor-pointer">
                          <span className="text-[10px] shrink-0 transition-colors duration-200" style={{ color: isActive || isHovered ? album.color : "var(--palette-4b5563)" }}>{(idx + 1).toString().padStart(2, "0")}</span>
                          {track.logoUrl && <span className="relative w-7 h-7 shrink-0 rounded bg-[var(--alpha-ffffff-05)] overflow-hidden"><Image src={track.logoUrl} alt="" fill sizes="28px" className="object-contain p-1" /></span>}
                          <span className={`text-sm font-semibold truncate transition-colors duration-200 ${isActive || isHovered ? "text-[var(--color-static-white)]" : "text-[var(--palette-6b7280)] group-hover/track:text-[var(--palette-d1d5db)]"}`}>{track.title}</span>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {track.spotifyUrl && <a href={track.spotifyUrl} target="_blank" rel="noreferrer" aria-label={`${track.title} Spotify`} className="text-[8px] font-bold text-[var(--palette-6b7280)] hover:text-[var(--color-static-white)] px-1.5 py-1">SP</a>}
                          {track.videoUrl && <a href={track.videoUrl} target="_blank" rel="noreferrer" aria-label={`${track.title} 뮤직비디오`} className="text-[8px] font-bold text-[var(--palette-6b7280)] hover:text-[var(--color-static-white)] px-1.5 py-1">MV</a>}
                        </div>
                        {isActive && isPlaying && (
            ...
            {activeTab === "members" && (
              <div className="absolute inset-0 animate-slideIn">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 h-full">
                  <p className="col-span-full self-center text-sm text-[var(--palette-6b7280)]">등록된 멤버 정보가 없습니다.</p>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="w-full py-3 border-t z-10 relative shrink-0"
        style={{ backgroundColor: "var(--alpha-080808-65)", backdropFilter: "blur(24px) saturate(1.3)", borderColor: "var(--alpha-ffffff-04)" }}
      >
        <div className="max-w-[1400px] mx-auto px-8 flex items-center gap-5">
          <button
            ...
            style={{ borderColor: "var(--alpha-ffffff-08)", backgroundColor: "var(--alpha-ffffff-03)" }}
          >
            <LuChevronLeft className="w-4 h-4 text-[var(--color-static-white)]" aria-hidden="true" />
          </button>
          ...
          <div ref={albumRailRef} className="discography-album-rail flex-1 flex items-center gap-2.5 overflow-x-auto scrollbar-none py-1">
              ...
              return (
                <button
                  ...
                  style={{
                    backgroundColor: isCurrent ? "var(--alpha-ffffff-06)" : undefined,
                    borderColor: isCurrent ? album.color : "transparent",
                    transition: "all 0.2s ease",
                  }}
                >
                  <div className="relative w-9 h-9 rounded-lg overflow-hidden border shrink-0 transition-all duration-200 group-hover:border-[var(--alpha-ffffff-2)]" style={{ borderColor: isCurrent ? `${a.color}60` : "var(--alpha-ffffff-06)" }}>
                  ...
                  <div className="text-left shrink-0 pr-1">
                    <p className={`text-[10px] font-black leading-none tracking-tight transition-colors duration-200 ${isCurrent ? "text-[var(--color-static-white)]" : "text-[var(--palette-6b7280)] group-hover:text-[var(--palette-e5e7eb)]"}`}>{a.title}</p>
                    <p className={`text-[7px] uppercase font-medium mt-0.5 tracking-wider transition-colors duration-200 ${isCurrent ? "text-[var(--palette-9ca3af)]" : "text-[var(--palette-4b5563)] group-hover:text-[var(--palette-6b7280)]"}`}>{a.type}</p>
                  </div>
                </button>
              );
            })}
          ...
          <button
            ...
            style={{ borderColor: "var(--alpha-ffffff-08)", backgroundColor: "var(--alpha-ffffff-03)" }}
          >
            <LuChevronRight className="w-4 h-4 text-[var(--color-static-white)]" aria-hidden="true" />
          </button>
        </div>
      </div>
    </main>
